...
Wire format intent:
  ...
  - lift_type values use the Swift `LiftType` raw values
    (chair_lift / gondola / funicular / t-bar / etc).
  - expected_trail_count / expected_lift_count mean reviewed, unique,
    routable canonical identities. They never mean a resort's headline
    marketing statistics, which may count segments, zones, or activities.
"""
...
class CanonicalManifest:
    """A reviewed manifest ready for the database wire format.

    `expected_*_count` is the number of unique canonical identities in
    `trails` / `lifts`, not a public resort headline statistic.
    """
    resort_id: str
...
class SourceResult:
    ...
    `candidate` sources expose routable source geometry, `inventory` sources
    are operator-curated identity lists, and `evidence` sources corroborate
    labels without claiming completeness or topology.
    """
    source: str                      # "skimap" / "openskimap" / "overpass" / "official"
    resort_id: str
    role: Literal["candidate", "inventory", "evidence"] = "candidate"
...
class DraftManifest:
    ...
    The database-compatible `expected_*_count` fields are canonical identity
    targets. `canonical_counts_reviewed` is a separate, fail-closed operator
    gate. Headline statistics and source references are retained only as
    review evidence and never participate in apply validation.
    """
    resort_id: str
    expected_trail_count: int
    expected_lift_count: int
    ...
    validator_notes: Optional[str] = None
    canonical_counts_reviewed: bool = False
    headline_trail_count: Optional[int] = None
    headline_lift_count: Optional[int] = None
...
class DraftRow:
    ...
    source_attribute_observations: Dict[str, Dict[str, Tuple[str, ...]]] = field(
        default_factory=dict
    )
    source_evidence_fingerprint: Optional[str] = None
