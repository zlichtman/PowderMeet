...
REQUEST_TIMEOUT = 120
ROUTABLE_AERIALWAYS = frozenset({
    "chair_lift", "gondola", "cable_car", "drag_lift", "t-bar", "j-bar",
    "platter", "rope_tow", "magic_carpet", "funicular", "mixed_lift",
})
...
def fetch(resort_id: str, hints: Optional[Dict[str, object]] = None) -> SourceResult:
    ...
    bbox = hints.get("bbox")
    if not bbox:
        raise ValueError("overpass source requires a bbox hint")

    south, west, north, east = bbox  # type: ignore[misc]
    cache_path = CACHE_DIR / f"{resort_id}.json"
    offline_fixtures = bool(hints.get("offline_fixtures", False))
    cached = _read_cached_json(cache_path, allow_stale=offline_fixtures)
    if cached is not None:
        data = cached
    elif offline_fixtures:
        raise RuntimeError(
            f"offline Overpass fixture is missing or invalid: {cache_path}"
        )
    else:
...
def _post_overpass(query: str) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"Overpass HTTP {err.code}") from err
    except (URLError, ValueError) as err:
        raise RuntimeError(f"Overpass request failed: {err}") from err
...
def _extract_items(data: dict) -> List[SourceItem]:
    ...
    for way in ways:
        ...
        # A piste area is outline evidence, not a routable identity/centerline.
        if tags.get("ski", tags.get("access")) in ("private", "no"):
            continue
        if tags.get("area") == "yes":
            continue
        aerialway = str(tags.get("aerialway") or "").lower()
        if aerialway in ROUTABLE_AERIALWAYS:
            kind = "lift"
        ...
        if kind == "trail":
            ...
            extra["is_groomed"] = normalize_grooming(tags.get("piste:grooming"))
        else:
...
def _read_cached_json(path: Path, *, allow_stale: bool = False) -> Optional[object]:
    if not path.exists():
        return None
