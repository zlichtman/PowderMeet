-- Keep corrupt, non-finite, or display-only activity metrics out of the
-- personalized per-edge speed model. The imported_runs log intentionally
-- retains unmatched/display-only rows, so this belongs at the learning
-- boundary rather than as a table constraint.
...
begin
  ...
  with eligible as (
    ...
    where r.profile_id = uid
      ...
      -- The upper bounds also reject PostgreSQL NaN and +Infinity.
      and r.speed_ms > 0
      and r.speed_ms <= 30
      and r.duration_s > 0
