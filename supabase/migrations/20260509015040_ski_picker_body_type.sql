-- Ski catalog: searchable list of brand/model entries shown in the picker.
-- Brand-only would be too coarse; model lets the algorithm later distinguish
-- "Atomic Bent 110" (powder skier) from "Atomic Redster" (carver) on the
-- same brand. waist_width_mm is captured for the future algorithm pass —
-- a 110mm waist is a different terrain preference than a 75mm waist.
...
-- Profile additions: body metrics (HealthKit-prefilled, user-editable) and
-- chosen ski. preferred_ski_id is FK to catalog with on-delete-set-null so
-- catalog cleanup never orphans a profile row.
...
-- Seed the catalog with a curated set covering major brands and category
-- spread. Friction-free first-launch picker depends on a non-empty catalog.
insert into public.skis_catalog (brand, model, category, waist_width_mm) values
  ...
  ('PowderMeet',  'House',             'all-mountain', 100)
