-- Atomic single-call manifest apply.
--
-- Replaces the multi-POST insert path in tools/canonical_ingest/apply.py
-- (manifest insert → trail bulk → lift bulk) which has a partial-failure
-- window if the second/third call errors after the first commits.
...
declare
  ...
  v_lift  jsonb;
begin
  -- Determine next manifest_version: 1 if no prior, else current + 1.
  ...
  -- Insert trails. Tolerate null / missing optional fields via ->> with coalesce.
...
-- Lock down to service_role only. Anonymous + authenticated callers
-- have no business writing manifests directly; canonical_ingest is the
-- only legitimate call site and it runs with the service-role key.
