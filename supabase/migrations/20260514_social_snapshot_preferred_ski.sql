-- Include preferred_ski_id in get_social_snapshot friend summaries.
--
-- FriendService applies snapshot friends as authoritative `UserProfile` rows.
-- Without this key, each snapshot refresh nils `preferredSkiId`, so friend ski
-- topsheets intermittently disappear in the friends list.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           ...
           'preferred_ski_id',   p.preferred_ski_id
         )), '[]'::jsonb)
    into friends_json
