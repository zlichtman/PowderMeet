...
// 20260502082535_device_tokens_and_push.sql). Receives:
//
//   { user_id: string, kind: string, payload: object }
//
// Looks up every device_tokens row for that user, groups by environment
// ('sandbox' for dev / Xcode-installed builds, 'production' for
// TestFlight + App Store), and POSTs to the matching APNs HTTP/2
// endpoint with that environment's signed JWT. Per-token environment
// routing means the same Supabase project serves dev and TestFlight
// users simultaneously without flipping a global env var.
//
// Required environment variables (set via `supabase secrets set`):
//
//   APNS_AUTH_KEY_SANDBOX     — full PEM body of the Sandbox-scoped .p8
//   APNS_KEY_ID_SANDBOX       — 10-character Key ID from the Sandbox key
//   APNS_AUTH_KEY_PRODUCTION  — full PEM body of the Production-scoped .p8
//   APNS_KEY_ID_PRODUCTION    — 10-character Key ID from the Production key
//   APNS_TEAM_ID              — 10-character Team ID (shared)
...
// ────────────────────────────────────────────────────────────────────
//
// Each environment has its own .p8 + Key ID (Apple split env-scoped
// keys; one key authenticates only to its environment's APNs server).
// JWTs are cached per environment, refreshed ~50 minutes in to leave
// safety margin within Apple's 60-minute validity window.

interface CachedJwt { jwt: string; exp: number }
const cachedTokens: Partial<Record<ApnsEnvironment, CachedJwt>> = {}

function authKey(env: ApnsEnvironment): { keyId: string; pem: string } {
  if (env === 'sandbox') {
    return {
      keyId: Deno.env.get('APNS_KEY_ID_SANDBOX')!,
      pem:   Deno.env.get('APNS_AUTH_KEY_SANDBOX')!
    }
  }
  return {
    keyId: Deno.env.get('APNS_KEY_ID_PRODUCTION')!,
    pem:   Deno.env.get('APNS_AUTH_KEY_PRODUCTION')!
  }
}

async function apnsAuthToken(env: ApnsEnvironment): Promise<string> {
  const now = Math.floor(Date.now() / 1000)
  const cached = cachedTokens[env]
  if (cached && cached.exp > now + 600) {
    return cached.jwt
  }

  const teamId = Deno.env.get('APNS_TEAM_ID')!
  const { keyId, pem } = authKey(env)

  const header  = { alg: 'ES256', kid: keyId, typ: 'JWT' }
  const payload = { iss: teamId, iat: now }
  const jwt     = await signES256(header, payload, pem)
  cachedTokens[env] = { jwt, exp: now + 50 * 60 }
  return jwt
}
...
Deno.serve(async (req: Request) => {
  ...
  // Sign one JWT per environment that actually has tokens — avoids
  // signing twice when only sandbox or only production is in use.
  const envsInUse: ApnsEnvironment[] = []
  if (iosTokens.some(t => t.environment === 'sandbox'))    envsInUse.push('sandbox')
  if (iosTokens.some(t => t.environment === 'production')) envsInUse.push('production')
  const jwts: Partial<Record<ApnsEnvironment, string>> = {}
  for (const env of envsInUse) {
    jwts[env] = await apnsAuthToken(env)
  }

  const results = await Promise.allSettled(
    iosTokens.map(async ({ token, environment }) => {
      const env: ApnsEnvironment = environment === 'production' ? 'production' : 'sandbox'
      const jwt = jwts[env]
      if (!jwt) throw new Error(`no jwt available for ${env}`)
      const res = await fetch(`${APNS_HOST[env]}/3/device/${token}`, {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(`apns ${env} ${res.status}: ${text}`)
      }
