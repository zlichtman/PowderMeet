...
// checkpoint blob so each call only has to process ~600 coords.
//
...
//   │ continue    │  → process next ~600 coords; merge into checkpoint;
...
import { fetchElevationBatch } from "../_shared/elevation_fetch.ts";
import { fetchOverpassSource, resortSourceQuery } from "../_shared/overpass_fetch.ts";
...
async function fetchOverpass(
  ...
  const source = await fetchOverpassSource(resortSourceQuery(south, west, north, east));
  // This is extraction provenance, not canonical review or live operating status.
  return { ...source, powdermeet_source_query_version: 2 } as OverpassResponse;
}
...
async function fetchElevations(coordKeys: string[]): Promise<Record<string, number>> {
  ...
  for (let i = 0; i < coordKeys.length; i += ELEVATION_BATCH) {
    ...
    const values = await fetchElevationBatch(batch, fetch, sleep, deadline);
    batch.forEach((key, index) => { result[key] = values[index]; });
    if (i + ELEVATION_BATCH < coordKeys.length) await sleep(350);
  }
