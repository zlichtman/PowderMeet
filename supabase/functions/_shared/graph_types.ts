...
export interface EdgeAttributes {
  ...
  weekdayWaitMinutes: number | null;
  weekendWaitMinutes: number | null;
  // True only on the physical lift's entry segment. Null preserves the
  // pre-v11 wire meaning (legacy lift edges charge one queue).
  chargesLiftWait: boolean | null;
...
export interface MountainGraphWire {
  ...
  edges: GraphEdgeWire[];
  fingerprint: string;
  rendezvousCatalog?: RendezvousCatalogWire;
}
...
export interface RendezvousPointWire {
  id: string;
  nodeID: string;
  kind: RendezvousKind;
...
export interface RendezvousCatalogWire {
  points: RendezvousPointWire[];
}
