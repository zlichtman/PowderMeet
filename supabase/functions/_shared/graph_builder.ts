...
//   4. pruneIsolatedNodes (named-edge nodes preserved)
//   5. assignTrailGroups (stable IDs from connected source edge IDs)
//   6. computeFingerprint (matches MountainGraph.computeFingerprint)
//
// Proximity/intersection/bridge repairs are intentionally excluded. Without
// curated evidence, a straight inferred link may cross a cliff, rope line, or
// grade-separated trail. Incomplete source topology fails closed.
...
export interface ResortDataCoordinate {
  lat: number;
  lon: number;
  ele?: number | null;
...
export interface ResortDataTrail {
  ...
  coordinates: ResortDataCoordinate[];
  lengthMeters: number;
...
export interface ResortDataConnection {
  id: string;
  name?: string | null;
  coordinates: ResortDataCoordinate[];
  isOpen: boolean;
...
export interface ResortData {
  ...
  connections?: ResortDataConnection[];
...
export function buildGraph(resort: ResortData, resortID: string): MountainGraph {
  ...
  let edges: GraphEdge[] = [];
  const sourceTopology = new Map<string, SourceTopology>();
  ...
  for (const trail of resort.trails) {
    ...
    const ownerID = `trail:${trail.id}`;
    const startNodeID = sourceEndpointNodeID(startCoord, ownerID, "start");
    const endNodeID = sourceEndpointNodeID(endCoord, ownerID, "end");
    ...
    sourceTopology.set(edgeID, {
      ownerID,
      vertexKeys: sourceVertexKeys(trail.coordinates, !goesDownhill),
      elevations: sourceElevations(trail.coordinates, !goesDownhill),
    });
  }
  ...
  for (const lift of resort.lifts) {
    ...
    const ownerID = `lift:${lift.id}`;
    const baseNodeID = sourceEndpointNodeID(baseCoord, ownerID, "base");
    const topNodeID = sourceEndpointNodeID(topCoord, ownerID, "top");
    ...
    if (lift.isBidirectional) reverseEntries.set(ownerID, topNodeID);
    const edgeID = `l${lift.id}`;
    edges.push({
      id: edgeID,
      sourceID: baseNodeID,
      targetID: topNodeID,
    ...
    sourceTopology.set(edgeID, {
      ownerID,
      vertexKeys: sourceVertexKeys(lift.coordinates, isReversed),
      elevations: sourceElevations(lift.coordinates, isReversed),
    });
  }

  // ── 3. Explicit source connectors → bidirectional traverse edges ──
  // These are sourced piste:type=connection ways, not inferred straight lines.
  for (const connection of resort.connections ?? []) {
    if (connection.coordinates.length < 2) continue;
    const start = connection.coordinates[0];
    const end = connection.coordinates[connection.coordinates.length - 1];
    const startEle = start.ele ?? 0;
    const endEle = end.ele ?? 0;
    const ownerID = `connection:${connection.id}`;
    const startID = sourceEndpointNodeID(start, ownerID, "start");
    const endID = sourceEndpointNodeID(end, ownerID, "end");
    ensureNode(nodes, startID, start, startEle, "junction");
    ensureNode(nodes, endID, end, endEle, "junction");

    const forwardGeometry: Coord[] = connection.coordinates.map((c) => [c.lon, c.lat]);
    const reverseGeometry = [...forwardGeometry].reverse();
    const length = polylineLength(forwardGeometry);
    const gain = endEle - startEle;
    const gradient = length > 0 ? Math.atan(Math.abs(gain) / length) * 180 / Math.PI : 0;
    const common = {
      trailName: connection.name ?? null,
      lengthMeters: length,
    ...
    const forwardID = `c${connection.id}_f`;
    const reverseID = `c${connection.id}_r`;
    edges.push({
      id: forwardID,
      sourceID: startID,
      targetID: endID,
      ...
      attributes: makeEdgeAttributes({ ...common, verticalDrop: gain }),
    });
    edges.push({
      id: reverseID,
      sourceID: endID,
    ...
    sourceTopology.set(forwardID, {
      ownerID,
      vertexKeys: sourceVertexKeys(connection.coordinates, false),
      elevations: sourceElevations(connection.coordinates, false),
    });
    sourceTopology.set(reverseID, {
      ownerID,
      vertexKeys: sourceVertexKeys(connection.coordinates, true),
      elevations: sourceElevations(connection.coordinates, true),
    });
  }
  ...
  // ── Safe source-topology pipeline ──
  edges = splitEdgesAtSharedSourceVertices(nodes, edges, sourceTopology);
  edges = splitLongEdges(nodes, edges, splitLength, sourceTopology);
  // Reverse only explicitly bidirectional lifts, after source segmentation.
  // Each direction pays a queue once at its own boarding end.
  const reverseEdges: GraphEdge[] = [];
  for (const edge of edges) {
    if (edge.kind !== "lift") continue;
    const owner = sourceTopology.get(edge.id)?.ownerID;
    const boarding = owner ? reverseEntries.get(owner) : undefined;
    if (!boarding) continue;
    const entry = edge.targetID === boarding;
    reverseEdges.push({ ...edge, id: `${edge.id}_rev`,
      sourceID: edge.targetID, targetID: edge.sourceID, geometry: [...edge.geometry].reverse(),
      attributes: { ...edge.attributes, chargesLiftWait: entry,
        waitTimeMinutes: entry ? edge.attributes.waitTimeMinutes : null,
        weekdayWaitMinutes: entry ? edge.attributes.weekdayWaitMinutes : null,
        weekendWaitMinutes: entry ? edge.attributes.weekendWaitMinutes : null } });
  }
  ...
  const fingerprint = computeFingerprint(resortID, nodesObj, edges);
...
function sourceVertexNodeID(sourceNodeID: string): string {
  return `src:${sourceNodeID}`;
}

function sourceEndpointNodeID(
  coordinate: ResortDataCoordinate,
  ownerID: string,
  endpoint: string,
): string {
  const sourceNodeID = coordinate.sourceNodeID?.trim();
  if (sourceNodeID) return sourceVertexNodeID(sourceNodeID);
  // Missing source identity is never permission to merge coincident ways.
  // Keep the fallback scoped to this source owner and endpoint.
  return `local:${ownerID.length}:${ownerID}:${endpoint}:${nodeID(coordinate)}`;
}
...
function ensureNode(
  ...
  if (existing) {
    if (kindPriority(kind) > kindPriority(existing.kind)) {
      nodes.set(id, {
        ...existing,
        elevation: elevation !== 0 || existing.elevation === 0 ? elevation : existing.elevation,
        kind,
      });
    }
    return;
...
function makeEdgeAttributes(p: Partial<EdgeAttributes>): EdgeAttributes {
  return {
    ...
    weekdayWaitMinutes: p.weekdayWaitMinutes ?? null,
    weekendWaitMinutes: p.weekendWaitMinutes ?? null,
    chargesLiftWait: p.chargesLiftWait ?? null,
...
export function computeAspect(coords: Coord[]): [number, number] {
  ...
  // Floating-point normalization can put R just above one for a straight segment.
  return [meanBearing, Math.max(0, Math.min(1, 1.0 - R))];
}

function estimateLiftTime(length: number, type: LiftType): number {
  let speed: number;
  switch (type) {
    case "cable_car": case "funicular": speed = 8.0; break;
    case "gondola":                     speed = 5.0; break;
    case "chair_lift":                  speed = 2.5; break;
    case "t-bar": case "j-bar":
    case "drag_lift":                   speed = 3.0; break;
    case "platter":                     speed = 2.5; break;
    case "rope_tow":                    speed = 2.0; break;
    case "magic_carpet":                speed = 1.0; break;
...
function sourceElevations(coordinates: ResortDataCoordinate[], reverse: boolean): Array<number | null> {
  const values = coordinates.map((c) => c.ele != null && Number.isFinite(c.ele) ? c.ele : null);
  return reverse ? values.reverse() : values;
}

// Keep measured elevations. Fill only missing samples between the nearest
// known anchors by traveled distance, never by vertex density.
function elevationProfile(
  geometry: Coord[], raw: Array<number | null> | undefined, from: number, to: number,
): number[] {
  const values = geometry.map((_, i) => raw?.[i] ?? null);
  if (values.length === 0) return [];
  values[0] = from;
  values[values.length - 1] = to;
  const distances = [0];
  for (let i = 1; i < geometry.length; i++) {
    distances.push(distances[i - 1] + haversineMeters(
      { lon: geometry[i - 1][0], lat: geometry[i - 1][1] },
      { lon: geometry[i][0], lat: geometry[i][1] },
    ));
  }
  let left = 0;
  for (let right = 1; right < values.length; right++) {
    ...
    for (let i = left + 1; i < right; i++) {
      const fraction = span > 0 ? (distances[i] - distances[left]) / span : 0;
      values[i] = values[left]! + (values[right]! - values[left]!) * fraction;
    }
    left = right;
  }
...
function profileMaximumGradient(geometry: Coord[], elevations: number[]): number {
  return computeMaxGradient(geometry.map((c, i) => ({ lon: c[0], lat: c[1], ele: elevations[i] })));
}

function sourceVertexKeys(
  coordinates: ResortDataCoordinate[],
  reverse: boolean,
): Array<string | null> {
  const keys = coordinates.map((coordinate) => {
    const sourceNodeID = coordinate.sourceNodeID?.trim();
    return sourceNodeID ? sourceVertexNodeID(sourceNodeID) : null;
  });
  return reverse ? keys.reverse() : keys;
}

// Split only at vertices that two distinct source ways explicitly share.
// Exact source node identity is stronger evidence than proximity or a visual
// line crossing and cannot accidentally connect a bridge to terrain below it.
function splitEdgesAtSharedSourceVertices(
  nodes: Map<string, GraphNode>,
  edges: GraphEdge[],
  topology: Map<string, SourceTopology>,
): GraphEdge[] {
  const ownersByVertex = new Map<string, Set<string>>();
  for (const source of topology.values()) {
    const uniqueKeys = new Set(
      source.vertexKeys.filter((value): value is string => value != null),
    );
    for (const key of uniqueKeys) {
      let owners = ownersByVertex.get(key);
      if (!owners) {
        owners = new Set<string>();
        ownersByVertex.set(key, owners);
      }
      owners.add(source.ownerID);
    }
  }
  const shared = new Set(
    [...ownersByVertex.entries()]
      .filter(([, owners]) => owners.size > 1)
      .map(([key]) => key),
  );
  if (shared.size === 0) return edges;

  const result: GraphEdge[] = [];
  for (const edge of edges) {
    const source = topology.get(edge.id);
    if (!source || source.vertexKeys.length !== edge.geometry.length) {
      result.push(edge);
      continue;
    }
    const splitIndices = [0];
    for (let index = 1; index < source.vertexKeys.length - 1; index++) {
      const key = source.vertexKeys[index];
      if (key != null && shared.has(key)) splitIndices.push(index);
    }
    splitIndices.push(edge.geometry.length - 1);
    const ordered = [...new Set(splitIndices)].sort((a, b) => a - b);
    if (ordered.length <= 2) {
      result.push(edge);
      continue;
    }

    const sourceElevation = nodes.get(edge.sourceID)?.elevation ?? 0;
    const targetElevation = nodes.get(edge.targetID)?.elevation ?? 0;
    const elevations = elevationProfile(edge.geometry, source.elevations, sourceElevation, targetElevation);
    const nodeIDs = ordered.map((index, position) => {
      if (position === 0) return edge.sourceID;
      if (position === ordered.length - 1) return edge.targetID;
      const coordinate = edge.geometry[index];
      const id = source.vertexKeys[index]!;
      ensureNode(
        nodes,
        id,
    ...
    for (let segment = 1; segment < ordered.length; segment++) {
      const geometry = edge.geometry.slice(ordered[segment - 1], ordered[segment] + 1);
      const segmentElevations = elevations.slice(ordered[segment - 1], ordered[segment] + 1);
      if (geometry.length < 2 || nodeIDs[segment - 1] === nodeIDs[segment]) continue;
      const length = polylineLength(geometry);
      const fromElevation = nodes.get(nodeIDs[segment - 1])?.elevation ?? 0;
      const toElevation = nodes.get(nodeIDs[segment])?.elevation ?? 0;
      const elevationChange = edge.kind === "traverse"
        ? toElevation - fromElevation
        : Math.abs(toElevation - fromElevation);
      const gradient = length > 0
        ? Math.atan(Math.abs(elevationChange) / length) * 180 / Math.PI
        : 0;
      const isLiftQueueEntry = edge.kind === "lift" && segment === 1;
      result.push({
        ...edge,
        ...
        attributes: makeEdgeAttributes({
          ...
          averageGradient: gradient,
          maxGradient: profileMaximumGradient(geometry, segmentElevations),
          midpointElevation: (fromElevation + toElevation) / 2,
          rideTimeSeconds: edge.attributes.rideTimeSeconds == null
            ? null
            : edge.attributes.rideTimeSeconds *
              (edge.attributes.lengthMeters > 0
                ? length / edge.attributes.lengthMeters
                : 1 / (ordered.length - 1)),
          waitTimeMinutes: edge.kind === "lift"
            ? (isLiftQueueEntry ? edge.attributes.waitTimeMinutes : null)
            : edge.attributes.waitTimeMinutes,
          weekdayWaitMinutes: edge.kind === "lift"
            ? (isLiftQueueEntry ? edge.attributes.weekdayWaitMinutes : null)
            : edge.attributes.weekdayWaitMinutes,
          weekendWaitMinutes: edge.kind === "lift"
            ? (isLiftQueueEntry ? edge.attributes.weekendWaitMinutes : null)
            : edge.attributes.weekendWaitMinutes,
          chargesLiftWait: edge.kind === "lift"
            ? isLiftQueueEntry
            : edge.attributes.chargesLiftWait,
        }),
      });
      topology.set(`${edge.id}_vx${segment}`, {
        ownerID: source.ownerID,
        vertexKeys: source.vertexKeys.slice(ordered[segment - 1], ordered[segment] + 1),
        elevations: segmentElevations,
      });
    }
  }
  return result;
...
function splitLongEdges(
  ...
  for (const edge of edges) {
    ...
    const elevations = elevationProfile(coords, topology.get(edge.id)?.elevations, srcEle, tgtEle);
    ...
    for (let i = 1; i < splitIndices.length - 1; i++) {
      ...
      // Resolution-only nodes are edge scoped. Reusing a coordinate-derived
      // ID here would reconnect unrelated ways that merely cross visually.
      const midNodeId = `${nodeID({ lat: midCoord[1], lon: midCoord[0] })}_split_${edge.id}_${splitIndices[i]}`;
      if (midNodeId === edge.sourceID || midNodeId === edge.targetID) continue;
      const midEle = elevations[splitIndices[i]];
    ...
    for (let seg = 1; seg < deduped.length; seg++) {
      ...
      newEdges.push({
        ...
        attributes: makeEdgeAttributes({
          ...
          maxGradient: profileMaximumGradient(geom, elevations.slice(prevIdx, nextIdx + 1)),
...
export function stableTrailGroupID(edgeIDs: string[]): string {
  const bytes = new TextEncoder().encode(
    `trail-group-v1|${[...edgeIDs].sort().join("|")}`,
  );
...
function assignTrailGroups(
  ...
  for (const key of sortedKeys) {
    ...
    if (indices.length === 1) {
      const idx = indices[0];
      const attrs = { ...out[idx].attributes, trailGroupId: stableTrailGroupID([out[idx].id]) };
    ...
    for (const root of sortedRoots) {
      ...
      const gid = stableTrailGroupID(edgeIndices.map((idx) => out[idx].id));
  ...
  for (let i = 0; i < out.length; i++) {
    ...
    out[i] = {
      ...out[i],
      attributes: { ...out[i].attributes, trailGroupId: stableTrailGroupID([out[i].id]) },
    };
  }

  // Unnamed traverse component grouping
  return mergeUnnamedTraverseChainComponents(out);
}
...
function mergeUnnamedTraverseChainComponents(
  ...
  for (const root of sortedRoots) {
    ...
    const gid = stableTrailGroupID(idxs.map((idx) => out[idx].id));
 ...
 *  - v11 domain separation and length-prefixed UTF-8 strings
 *  - exact IEEE-754 Double bit patterns
 *  - explicit presence markers for every optional value
 *  - deterministic resort/node identity followed by sorted edge topology,
 *    geometry, and every EdgeAttributes field
 *  - final format: "<nodeCount>:<edgeCount>:<hex16>"
 *
 * Determinism note: both implementations sort nodes and edges by stable ID
 * before fingerprinting, so source insertion order cannot change identity.
 */
export function computeFingerprint(
  ...
  const mixString = (s: string) => {
    ...
    mix(BigInt(bytes.length));
    for (const b of bytes) mix(BigInt(b));
  };
  const doubleBuffer = new ArrayBuffer(8);
  const doubleView = new DataView(doubleBuffer);
  const mixDouble = (d: number) => {
    // JSON.stringify(-0) emits 0, so hash the wire-semantic value.
    doubleView.setFloat64(0, Object.is(d, -0) ? 0 : d, false);
    mix(doubleView.getBigUint64(0, false));
  };
  const mixOptionalDouble = (d: number | null | undefined) => {
    if (d == null) {
      mix(0n);
      return;
    }
  ...
  const mixOptionalString = (s: string | null | undefined) => {
    if (s == null) {
      mix(0n);
      return;
    }
  ...
  const mixInt = (value: number) => {
    const signed = BigInt(Math.trunc(value));
    mix(signed < 0n ? (signed + (1n << 64n)) & MASK : signed & MASK);
  };
  const mixOptionalInt = (value: number | null | undefined) => {
    if (value == null) {
      mix(0n);
      return;
    }
  ...
  const mixOptionalBool = (value: boolean | null | undefined) => {
    if (value == null) {
      mix(0n);
      return;
    }
  ...
  mixString(resortID);
  mix(BigInt(Object.keys(nodes).length));
  const sortedNodes = Object.values(nodes).sort((a, b) =>
    a.id < b.id ? -1 : a.id > b.id ? 1 : 0
  );
  for (const node of sortedNodes) {
    mixString(node.id);
    mixDouble(node.coordinate.lat);
    mixDouble(node.coordinate.lon);
    mixDouble(node.elevation);
    mixString(node.kind);
  }

  mix(BigInt(edges.length));
  for (const e of sortedEdges) {
    mixString(e.id);
    mixString(e.sourceID);
    mixString(e.targetID);
    mixString(e.kind);
    mix(BigInt(e.geometry.length));
    for (const coordinate of e.geometry) {
      mixDouble(coordinate[1]);
      mixDouble(coordinate[0]);
    }
    const a = e.attributes;
    mixOptionalString(a.difficulty);
