...
extension ContentCoordinator {
    ...
    func loadConditions(for entry: ResortEntry) {
        ...
        conditionsTask = Task { [weak self] in
            ...
            let merged = await ConditionsService.shared.mergeHourly(for: entry)
            guard !Task.isCancelled else { return }
            if let merged {
                self?.resortConditions = merged
            }

            // Re-evaluate an active route only after the hourly merge attempt
            // completes. Triggering on the fast current-only snapshot would
            // consume the controller's 30-second throttle just before the
            // forecast arrives, leaving the richer wind/visibility timeline
            // unapplied. The controller keeps the agreed meeting point fixed,
            // rejects same-path/timing noise, requires meaningful savings, and
            // does nothing when no meetup is active.
            self?.meetup.evaluateFasterRerouteIfNeeded()
        }
    }
