...
final class RealtimeBootstrapper {
    ...
    func ensureLocation() -> RealtimeLocationService {
        ...
        // Capture `[weak self]` and call `resortGraphProvider()` lazily
        // inside the closure instead of snapshotting it. If
        // `ensureLocation()` runs before `bind(resortManager:)` installs
        // the real provider — possible during cold-launch races, or any
        // future code path that builds the bootstrapper before binding —
        // the snapshot would freeze the resolver on the default
        // `{ nil }` and never recover. Lazy lookup picks up the real
        // provider on the first call after binding.
        rtl.nodeResolver = { [weak self] coord in
            let graph = self?.resortGraphProvider()
            return graph?.routingStartNode(to: coord)?.id
        }

        // Friend-only broadcast filter. See `AGENTS.md` (social snapshot
    ...
    ///
    /// Awaitable so the caller can sequence sign-out → channels-released
    /// → friend caches cleared. Without the await on
    /// `presence.stopAndWait`, a fast re-sign-in could overlap the old
    /// session's channels with the new session's.
    func teardown() async {
        await presence?.stopAndWait()
