...
nonisolated enum SolverConstants {
    ...
    enum Scoring {
        ...
        /// Soft wait-time penalty α — adds α·|tA−tB| to the `.fastest`
        /// score so ties at the same max(tA,tB) prefer balanced meets.
        /// 0.6 means a 60s imbalance costs 36s of total time, just above
        /// the secondaryFactor max (30s) so a hub bonus / mid-mountain
        /// elevation preference can't outvote a 60s+ imbalance. Was 0.3
        /// — at that weight, the secondary factors could pull the meet
        /// toward a "nicer" but lopsided node where one skier waits
        /// while the other is still mid-mountain. Hard 300s reject
        /// threshold below catches the worst regardless.
        static let waitPenaltyAlpha: Double = 0.6
        /// CVaR weight β on the joint-path stddev term in the time
        /// score. `score = max(tA, tB) + α|tA-tB| + β·sqrt(varA+varB)`.
    ...
    enum Alternates {
        ...
        /// Count of alternates for the two-skier solver. Bumped from
        /// 3 → 4 so the user sees more spatial variety on the option
        /// cards. `MeetingOptionsSection` renders the count
        /// dynamically via `ForEach`, no other coupling.
        ...
        /// (no haversine gate, lift-cluster only). 0 keeps the
        /// original behaviour as the last ladder rung. Below that,
        /// the solver falls back to the "no-constraint" pass that
        /// only dedupes by node id. Lift-zone + 150 m grid clustering
        /// stays in place as an additional axis at every rung except
