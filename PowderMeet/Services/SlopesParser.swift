...
nonisolated struct SlopesRun {
    ...
    /// True only for a modern Metadata.xml `Action type="Run"`. Legacy
    /// database/CSV fallbacks are whole-activity hints and must be segmented.
    let isAuthoritativeDownhillRun: Bool
}
            ...
            return .success(ParsedActivity(
                ...
                segments: activity.runs.map { run in
                    ParsedRunSegment(
                        ...
                        points: run.points,
                        boundary: run.isAuthoritativeDownhillRun
                            ? .authoritativeDownhillRun
                            : .providerHint
                    )
                }
            ))
    ...
    private static func buildActivity(
        ...
        for action in meta.runs {
            ...
            runs.append(SlopesRun(
                ...
                points: slice,
                isAuthoritativeDownhillRun: true
            ))
        }
    ...
    private static func syntheticActivity(points: [GPXTrackPoint]) -> SlopesActivity {
        ...
        return SlopesActivity(
            ...
            runs: [SlopesRun(
                ...
                verticalMeters: 0,
                points: points,
                isAuthoritativeDownhillRun: false
            )]
        )
    }
