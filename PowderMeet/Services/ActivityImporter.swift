...
nonisolated struct ActivityImporter {
    ...
    private func processFile(url: URL) async -> ProcessedFile {
        ...
        if format == .powdermeetBackup {
            ...
            return ProcessedFile(
                outcome: pf.outcome,
                matchedRuns: pf.matchedRuns,
                resortId: pf.resortId,
                graph: pf.graph,
        ...
        // Whole-file dedup intentionally removed: re-uploading the same
        // file now re-imports every run. The contract is "every run in
        // the file lands in the log", duplicates included.
    ...
    private func processParsed(_ parsed: ParsedActivity, label url: URL) async -> ProcessedFile {
        ...
        // HealthKit whole-source dedup intentionally removed too —
        // re-pulling Health re-imports every workout's runs.
    ...
    private func elevationSegment(
        ...
        var runIdx = 0
        for seg in segments where !seg.isLift && seg.points.count >= 2 {
            let start = seg.points.first?.timestamp ?? source.startTime
            let end = seg.points.last?.timestamp ?? source.endTime
            let duration = end.timeIntervalSince(start)
            // The old `duration > 25 min` drop is gone: the segmenter
            // now hard-splits on time gaps, so a long blob is no longer
            // two merged runs — and the user wants every run kept even
            // if one genuinely ran long. Only a near-stationary segment
            // (lunch with the watch running) is still discarded; that's
            // not a run anyone expects to see in the log.
            ...
            out.append(ParsedRunSegment(
                ...
                topSpeedMS: nil,
                avgSpeedMS: nil,
                distanceMeters: distanceFromPoints(seg.points),
                // Compute vertical from the GPS elevation track — descent
                // is the dominant signal in a ski run, so we sum negative
                // elevation deltas (start - end on each point pair) and
                // return the absolute total. Hardcoding nil here was the
                // "0 ft vertical" regression: persistRuns falls back to
                // `a.drop` from the matched edge, which is (0,0,0) for
                // any run that didn't match the trail graph (off-piste,
                // sparse-GPS, out-of-catalog resort).
                verticalMeters: descentFromPoints(seg.points),
                points: seg.points
            ))
        }
    ...
    /// Sum of NEGATIVE elevation deltas (returned as a positive number)
    /// across the point sequence — vertical drop for a ski run. Filters
    /// barometric noise via a small dead-band (1 m) so a GPS waver of
    /// ±0.3 m on each sample doesn't accumulate into a phantom vertical.
    /// Returns nil only when no point has an elevation value (e.g.,
    /// HKWorkoutRoute samples on devices that didn't record altitude).
    private func descentFromPoints(_ points: [GPXTrackPoint]) -> Double? {
        let elev = points.compactMap { $0.elevation }
        guard elev.count >= 2 else { return nil }
        var drop = 0.0
        let deadband = 1.0
        for i in 1..<elev.count {
            let delta = elev[i - 1] - elev[i]
            if delta > deadband { drop += delta }
        }
        return drop
    }

    /// Total haversine distance across the point sequence in meters.
    /// Used by the elevation-segmenter so its synthesized run segments
    /// carry the same `measuredDistanceM` field native-parsed runs
    /// already populate (TCX/FIT laps); without it, persistRuns falls
    /// back to the matched-edge's nominal length, which is 0 m for any
    /// run that didn't match the trail graph.
    private func distanceFromPoints(_ points: [GPXTrackPoint]) -> Double? {
    ...
    private func persistRuns(_ runs: [MatchedRun], resortId: String, graph: MountainGraph?) async {
        ...
        let rows: [ImportedRunRow] = runs.map { run in
            ...
            if let edgeId = run.edgeId {
                ...
                // Unmatched runs all share the literal "unmatched" edge
                // placeholder, so two distinct unmatched runs landing in
                // the same 15s start bucket used to collide and the
                // second was silently dropped by `ignoreDuplicates`.
                // Add a duration discriminator (10s rounding) so e.g.
                // two side-by-side off-piste laps with different lengths
                // hash to different keys. Same-source re-export of the
                // same run still produces the same duration → same key
                // → still deduped (the intended behavior).
                let durBucket = Int((run.duration / 10).rounded())
                hash = "\(run.source.rawValue)|\(startBucket)|\(resortId)|unmatched|d\(durBucket)"
            }
        ...
        do {
            // Plain insert, not an ignoreDuplicates upsert. There is no
            // unique constraint on (profile_id, dedup_hash) — only a
            // plain index — so this is safe, and it means every parsed
            // run is written even when its dedup_hash collides with an
            // existing row (same-minute laps, re-imports). The user's
            // contract: every run in the file shows in the log.
            try await supabase.client
                .from("imported_runs")
                .insert(rows)
                .execute()
        } catch {
            AppLog.importer.error("imported_runs insert failed: \(error.localizedDescription)")
        }
    }
    ...
    @MainActor
    private func recomputeProfileStatsRow(userId: UUID) async -> Bool {
        do {
            try await supabase.client
    ...
    private func logPerFileTelemetry(_ pf: ProcessedFile) {
    ...
    private func logBatchTelemetry(processed: [ProcessedFile], recomputeSucceeded: Bool, elapsedMs: Int) {
    ...
    private func elapsedMs(since startNanos: UInt64) -> Int {
