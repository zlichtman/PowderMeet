...
nonisolated enum SlopesMetadataParser {
    ...
    static func parse(data: Data) -> Result<SlopesMetadata, SlopesMetadataError> {
        ...
        guard delegate.invalidActionCount == 0 else {
            return .failure(.malformed(
                "\(delegate.invalidActionCount) <Action> element(s) had invalid boundaries"
            ))
        }
        let orderedActions = delegate.actions.sorted {
            ($0.start, $0.runNumber) < ($1.start, $1.runNumber)
        }
        let parsedRunCount = orderedActions.filter(\.isRun).count
        guard header.runCount == 0 || header.runCount == parsedRunCount else {
            return .failure(.malformed(
                "Activity declared \(header.runCount) runs but \(parsedRunCount) were decoded"
            ))
        }
        return .success(SlopesMetadata(
            header: header,
    ...
    private final class Delegate: NSObject, XMLParserDelegate {
        ...
        private func makeHeader(from a: [String: String]) -> SlopesActivityHeader {
            SlopesActivityHeader(
                ...
                durationSeconds: nonnegativeValue(a["duration"]),
                distanceMeters: nonnegativeValue(a["distance"]),
                verticalMeters: nonnegativeValue(a["vertical"]),
                topSpeedMS: nonnegativeValue(a["topSpeed"]),
        ...
        private func makeAction(from a: [String: String]) -> SlopesActionMetadata? {
            guard let start = SlopesDate.parse(a["start"]),
                  let end = SlopesDate.parse(a["end"]),
                  end > start else {
                return nil
            }
            ...
            let measuredDuration = end.timeIntervalSince(start)
            let declaredDuration = doubleValue(a["duration"])
            let duration = declaredDuration.flatMap {
            ...
            return SlopesActionMetadata(
                ...
                topSpeedMS: nonnegativeValue(a["topSpeed"]),
                avgSpeedMS: nonnegativeValue(a["avgSpeed"]),
                distanceMeters: nonnegativeValue(a["distance"]),
                verticalMeters: nonnegativeValue(a["vertical"]),
        ...
        private func nonnegativeValue(_ s: String?) -> Double {
            guard let value = doubleValue(s), value >= 0 else { return 0 }
            return value
        }

        private func localName(_ elementName: String) -> String {
            guard let colon = elementName.lastIndex(of: ":") else {
                return elementName
            }
...
/// Imports run concurrently, so each call owns its formatter.
