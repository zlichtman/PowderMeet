...
//
//  Garmin Connect, Wahoo, and Strava may emit one or more `<Lap>`
//  containers. A lap is treated as a provider hint, not assumed to be a
//  physical ski run; the shared segmenter validates/splits it later. A lap
//  can carry
...
nonisolated final class TCXParser: NSObject, XMLParserDelegate {
    ...
    static func parse(data: Data) -> [GPXTrack] {
        ...
        guard parser.parse() else {
            // A streaming parser can emit an earlier Lap before a malformed
            // suffix is discovered. Reject the whole file instead of
            // presenting that partial activity as complete.
            return []
        }
    ...
    /// `XMLParser` does not process namespaces by default, so prefixed TCX
    /// exports surface names such as `ns3:Trackpoint` and `ns3:Speed`.
    /// Normalize every start/end event instead of special-casing vendors.
