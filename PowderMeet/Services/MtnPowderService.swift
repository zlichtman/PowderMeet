...
enum MtnPowderResortMapping: Sendable {
    ...
    /// Resolve only an explicit resort identity. Neighboring mountains are never substitutes.
    nonisolated static func resortID(for feedName: String) -> String? {
        let name = feedName.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !skipNames.contains(name) else { return nil }
        return nameToResortId[name]
    }
...
actor MtnPowderService {
    ...
    func fetchData(resortId: String) async -> MtnPowderData? {
        ...
        let matchingEntries = feedResorts.filter { entry in
            ...
            return MtnPowderResortMapping.resortID(for: name) == resortId
        }
