...
nonisolated final class GPXParser: NSObject, XMLParserDelegate {
    ...
    /// can emit one provider hint per `<trkseg>`. The shared physical
    /// segmenter later validates/splits these hints.
    ...
    static func parse(data: Data) -> [GPXTrack] {
        ...
        guard parser.parse() else {
            ...
            // XMLParser is streaming and may already have emitted complete
            // tracks before discovering malformed trailing XML. Never return
            // that partial activity as if the source file were trustworthy.
            return []
        }
    ...
    /// parser still emits one segment containing every point. Parsers stay
    /// format-only; `SkiActivitySegmenter` owns physical run boundaries.
    ///
