...
final class RealtimeLocationService {
    ...
    private(set) var transportState: LiveTransportState = .stopped
    /// Invalidates a suspended start when stop or a resort switch wins.
    ...
    struct FriendLocation: Sendable {
        ...
        /// Broadcast-only motion hints. Old clients and REST hydration leave
        /// these nil; fresh peers use them to distinguish nearby parallel or
        /// opposite-direction corridors without changing database schema.
        ...
        var usableTravelCourse: CLLocationDirection? {
            guard let speedMetersPerSecond,
                  ...
                  courseDegrees >= 0,
                  courseDegrees < 360 else { return nil }
        ...
        var routingAltitudeMeters: Double? {
            guard let altitudeMeters,
                  ...
                  verticalAccuracyMeters <= 50 else { return nil }
    ...
    struct PositionPayload: Codable, Sendable {
        ...
        let crs: Double?    // moving course degrees; nil when unavailable
        let spd: Double?    // ground speed m/s; validates whether crs is useful
        let alt: Double?    // altitude meters; broadcast-only routing hint
        let vacc: Double?   // vertical accuracy meters
    ...
    ///   "accept-everyone-during-cold-launch" window per `AGENTS.md`
    ...
    init(supabase: SupabaseManager? = nil,
        ...
        // Disk hydration waits until `start(resortId:)` so cached coordinates
        // can be scoped to the mountain being displayed.
    ...
    func start(resortId: String) async {
        ...
        if case .live(let current) = transportState, current == resortId {
            // The lifecycle is live but one transport leg is absent. Repair
            // channels in-place instead of starting a second ownership cycle.
            await reconnectPositionChannelsIfActive()
            return
        }
        guard transportState.canStart(resortID: resortId) else {
            AppLog.realtime.debug("start ignored in state \(String(describing: transportState))")
            return
        }

        transportGeneration &+= 1
        let generation = transportGeneration
        transportState = .connecting(resortID: resortId)
        ...
        // Clear stale cross-resort friend entries from memory, then hydrate
        // only last-known rows stamped for this exact mountain.
        ...
        if let stored = persistentStore?.loadAll(resortId: resortId) {
            for location in stored {
                friendLocations[location.userId] = location
            }
        }

        // Continue this visible ski session with the system location indicator.
        // When-In-Use permission is sufficient; never ask for an Always upgrade.
        ...
        transportState = .live(resortID: resortId)
    ...
    func reconnectPositionChannelsIfActive() async {
        guard let userId = supabase.currentSession?.user.id,
              ...
              transportState == .live(resortID: resortId) else { return }
    ...
    private func performBroadcast(force: Bool) async {
        ...
        let capturedAt = testLocationOverride == nil
            ? locationManager.currentFixTimestamp
            : now
        guard let capturedAt,
              RoutingFixPolicy.isFresh(capturedAt: capturedAt, now: now) else {
            return
        }
        ...
        let hasTestOverride = testLocationOverride != nil
        let speed = !hasTestOverride && locationManager.currentSpeed >= 0
            ? locationManager.currentSpeed
            : nil
        let course = !hasTestOverride && isRoutingGrade
            ? locationManager.usableTravelCourse
            : nil
        let altitude = !hasTestOverride && isRoutingGrade
            ? locationManager.routingAltitudeMeters
            : nil
        let verticalAccuracy = altitude == nil
            ? nil
            : locationManager.currentVerticalAccuracy
        let nodeId = isRoutingGrade
            ? resolveNodeId(for: coord)
            : nil
    ...
    private func hydrateFromTable(userId: UUID, resortId: String) async {
        do {
            ...
            for row in rows {
                ...
                guard FriendSignalClassifier.isAcceptableLocationPayload(
                    latitude: row.lat,
                    longitude: row.lon,
    ...
    private func ensureSubscribedToCell(of coord: CLLocationCoordinate2D, resortId: String) async {
        guard currentResortId == resortId, !Task.isCancelled else { return }
        let cell = Geohash.encode(coordinate: coord, precision: 6)
    ...
    private func performCellSubscription(needed: Set<String>, resortId: String) async {
        ...
        for old in subscribedCells.subtracting(needed) {
            ...
            subscribedCells.remove(old)
        }

        // Update ownership incrementally. A stop can cancel us at any await;
        // every completed acquire is either visible in subscribedCells for the
        // teardown to release, or explicitly released before returning.
        for cellId in needed.subtracting(subscribedCells).sorted() {
            guard currentResortId == resortId, !Task.isCancelled else { return }
            let channelName = Self.channelName(for: cellId)
            do {
                _ = try await registry.acquire(name: channelName)
                guard currentResortId == resortId, !Task.isCancelled else {
                    await registry.release(name: channelName)
                    return
                }
                ...
                subscribedCells.insert(cellId)
            } catch {
                AppLog.realtime.error("failed to subscribe to cell \(cellId): \(error)")
            }
        }

        AppLog.realtime.debug("subscribed to cells: \(subscribedCells.sorted().joined(separator: ","))")
    }
    ...
    private func handleIncomingBroadcast(message: JSONObject, resortId: String) async {
        ...
        let loc = FriendLocation(
            ...
            courseDegrees: payload.crs,
            speedMetersPerSecond: payload.spd,
            altitudeMeters: payload.alt,
            verticalAccuracyMeters: payload.vacc
        )
    ...
    private func stopAsync() async {
        ...
        // Cancellation alone does not synchronously stop an async acquire.
        // Await it before snapshotting ownership so no channel can land after
        // the release loop and escape teardown.
        if let pending = pendingCellSubscription {
            pending.cancel()
            await pending.value
        }
    ...
    func stop() {
        ...
        transportGeneration &+= 1
        let generation = transportGeneration
        transportState = .stopping(resortID: currentResortId)
        heartbeatTask?.cancel(); heartbeatTask = nil
        let pendingCell = pendingCellSubscription
        pendingCell?.cancel()
        pendingCellSubscription = nil
        if pendingStopTask == nil {
            let registry = self.registry
            pendingStopTask = Task { @MainActor [weak self] in
                await pendingCell?.value
                guard let self else { return }
                let cells = self.subscribedCells
                self.subscribedCells.removeAll()
                let resortChannel = self.subscribedResortChannel
                self.subscribedResortChannel = nil
                for cell in cells {
                    await registry.release(name: Self.channelName(for: cell))
                }
            ...
            // An earlier release task is still the ownership barrier. Its
            // completion cannot stamp this newer generation, so the waiter
            // below will finish the transition.
        }
        locationManager.endSession()
        friendsPresent.removeAll()
        currentResortId = nil
    ...
    func waitForStop() async {
        await pendingStopTask?.value
        if case .stopping = transportState {
