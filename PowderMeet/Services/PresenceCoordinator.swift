...
//  window `AGENTS.md` (social snapshot gate + PresenceCoordinator) calls out.
//
...
final class PresenceCoordinator {
    ...
    /// Synchronous stop. Use for rapid resort-clears where you don't
    /// need to provably wait for channels to release — the underlying
    /// realtime service tears channels down off-main and another
    /// `enter()` will sequence behind that via its own teardown.
    /// For sign-out / account-deletion teardown, prefer
    /// `stopAndWait()` so a follow-on sign-in can't race the
    /// outgoing channels.
    func stop() {
        enterGeneration &+= 1
        currentEnterTask?.cancel()
    ...
    /// Awaitable stop. Cancels any in-flight enter, requests channel
    /// release on `RealtimeLocationService`, and waits for both the
    /// cancelled pipeline and the detached release task to finish
    /// before clearing state. Returning means: the channels this
    /// coordinator owned are provably released, and no still-running
    /// enter pipeline will write `phase = .live` after this point.
    /// Use this on sign-out so a fast re-sign-in (or account
    /// deletion → re-create) doesn't overlap the old session's
    /// channels with the new session's.
    func stopAndWait() async {
        let inFlight = currentEnterTask
        enterGeneration &+= 1
        currentEnterTask?.cancel()
        currentEnterTask = nil
        phase = .tearingDown
        ...
        // Awaiting the cancelled pipeline guarantees its gen-checks
        // have run and it returned without writing `phase = .live`
        // after we set `.tearingDown`.
