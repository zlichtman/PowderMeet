...
extension CuratedResortLoader {
    ...
    nonisolated private static func stripEdgeIdToOSMId(_ edgeId: String) -> String {
        ...
        // All builder suffixes follow the numeric physical OSM way identity.
        if let underscore = id.firstIndex(of: "_") {
            id = String(id[..<underscore])
        }
        return id
    }
    ...
    nonisolated static func applyOverlay(_ curated: CuratedResort, to graph: inout MountainGraph) {
        ...
        func matchedLift(_ edge: GraphEdge) -> CuratedLift? {
            liftsByOSMId[edge.id]
                ?? liftsByOSMId[stripEdgeIdToOSMId(edge.id)]
                ?? edge.attributes.trailName.flatMap { liftsByName[$0.lowercased()] }
        }
        func laneKey(_ edge: GraphEdge, _ lift: CuratedLift) -> String {
            lift.name.lowercased() + (edge.id.hasSuffix("_rev") ? ":reverse" : ":forward")
        }
        var laneLengths: [String: Double] = [:]
        for edge in graph.edges where edge.kind == .lift {
            guard let lift = matchedLift(edge) else { continue }
            laneLengths[laneKey(edge, lift), default: 0] += edge.attributes.lengthMeters
        }
        ...
        for i in graph.edges.indices {
            ...
            if edge.kind == .run {
                ...
                if let diff = ct.difficulty, let rd = RunDifficulty(rawValue: diff) {
                    graph.edges[i] = GraphEdge(
                        ...
                        attributes: EdgeAttributes(
                            ...
                            aspectVariance: edge.attributes.aspectVariance,
                            trailName: ct.name,
                            ...
                            weekdayWaitMinutes: edge.attributes.weekdayWaitMinutes,
                            weekendWaitMinutes: edge.attributes.weekendWaitMinutes,
                            chargesLiftWait: edge.attributes.chargesLiftWait,
                            isOpen: edge.attributes.isOpen,
                            isOfficiallyValidated: edge.attributes.isOfficiallyValidated,
                            trailGroupId: edge.attributes.trailGroupId,
                            midpointElevation: edge.attributes.midpointElevation,
                            estimatedTrailWidthMeters: edge.attributes.estimatedTrailWidthMeters,
                            obstacleDensity: edge.attributes.obstacleDensity,
                            fallLineExposure: edge.attributes.fallLineExposure,
                            nightGroomedFlag: edge.attributes.nightGroomedFlag,
                            lastGroomedHoursAgo: edge.attributes.lastGroomedHoursAgo,
                            estimatedSurfaceCondition: edge.attributes.estimatedSurfaceCondition
                        )
                    )
                    modified = true
                }
                ...
                guard let cl = matchedLift(edge) else { continue }
                let totalLength = laneLengths[laneKey(edge, cl)] ?? 0
                let fraction = totalLength > 0 ? edge.attributes.lengthMeters / totalLength : 1
                let isQueueEntry = edge.attributes.chargesLiftWait != false
                ...
                graph.edges[i] = GraphEdge(
                    ...
                    attributes: EdgeAttributes(
                        ...
                        verticalDrop: cl.verticalRise.map { $0 * fraction } ?? edge.attributes.verticalDrop,
                        averageGradient: edge.attributes.averageGradient,
                        maxGradient: edge.attributes.maxGradient,
                        aspect: edge.attributes.aspect,
                        aspectVariance: edge.attributes.aspectVariance,
                        ...
                        rideTimeSeconds: cl.rideTimeSeconds.map { $0 * fraction } ?? edge.attributes.rideTimeSeconds,
                        // Curated values are stable weekday/weekend baselines,
                        // not live observations. Keep the live slot untouched;
                        // traversal selects the correct baseline at the
                        // skier's resort-local edge-arrival day.
                        waitTimeMinutes: edge.attributes.waitTimeMinutes,
                        weekdayWaitMinutes: isQueueEntry
                            ? cl.weekdayWaitMinutes ?? edge.attributes.weekdayWaitMinutes : nil,
                        weekendWaitMinutes: isQueueEntry
                            ? cl.weekendWaitMinutes ?? edge.attributes.weekendWaitMinutes : nil,
                        chargesLiftWait: edge.attributes.chargesLiftWait,
                        isOpen: edge.attributes.isOpen,
                        isOfficiallyValidated: edge.attributes.isOfficiallyValidated,
                        trailGroupId: edge.attributes.trailGroupId,
