...
final class HealthKitImporter {
    ...
    private var readTypes: Set<HKObjectType> {
        ...
        // Body metrics — surfaced through the BODY TYPE sheet in the
        // Activity calibration menu. Prefilled from the most recent sample
        // when available; user-editable. The OS prompt for these is
        // bundled with the workout-type prompt the first time the user
        // opens BODY TYPE or imports activity, whichever comes first.
    ...
    /// Most-recent height sample in centimeters, or nil if no sample is
    /// available (or read access is denied — HealthKit deliberately returns
    /// the same empty result either way for privacy).
    func fetchLatestHeight() async -> Double? {
        await fetchLatestQuantity(.height, unit: .meterUnit(with: .centi))
    }

    /// Most-recent body-mass sample in kilograms, or nil if unavailable.
    ...
    private func fetchLatestQuantity(
        ...
        guard Self.isAvailable,
              let qType = HKObjectType.quantityType(forIdentifier: identifier) else {
            return nil
        }
        // Best-effort auth — if the user said no, the query just returns
        // empty; we don't want a thrown error to bubble up out of a
        // prefill helper.
        try? await requestAuthorization()
    ...
    private func parsedActivity(from workout: HKWorkout) async -> ParsedActivity? {
        ...
        // Raw-source identity anchors on the workout UUID. Including the
        // route sample count distinguishes a route that HealthKit back-filled
        // after the first import; per-run database identity handles exact
    ...
    func fetchLatestHeight() async -> Double? { nil }
