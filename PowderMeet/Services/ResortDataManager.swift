...
//  Never constructs routing topology from live device-specific Overpass data.
//
...
enum ResortLoadError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .datasetVersionMismatch:
            return "The exact mountain dataset requested is not available."
        }
    }
}

/// Exact identity of one resort-load request. Snapshot and canonical dataset
/// overrides are part of the key because two requests for the same resort can
/// still be asking for different immutable mountains.
nonisolated struct ResortLoadRequestKey: Hashable, Sendable {
    let resortID: String
    let snapshotOverride: String?
    let manifestVersionOverride: Int?
    let datasetVersionOverride: String?
    /// Bypasses in-memory graph/status projections so a foreground resume can
    /// fetch a fresh operational sidecar even when topology is unchanged.
    let forceOperationalRefresh: Bool
}

/// Small, pure latest-request-wins state machine used by
/// `ResortDataManager`. Network requests are not assumed to finish in start
/// order; only the latest ticket may publish graph, cache, loading, or error
/// state. An identical request already in flight is coalesced.
nonisolated struct ResortLoadArbiter: Sendable {
    nonisolated struct Ticket: Equatable, Sendable {
        fileprivate let generation: UInt64
        let key: ResortLoadRequestKey
    }

    private(set) var generation: UInt64 = 0
    private(set) var activeKey: ResortLoadRequestKey?

    mutating func begin(_ key: ResortLoadRequestKey) -> Ticket? {
        guard activeKey != key else { return nil }
        generation &+= 1
        activeKey = key
        return Ticket(generation: generation, key: key)
    }

    func isLatest(_ ticket: Ticket) -> Bool {
...
class ResortDataManager {
    ...
    /// Immutable topology/metadata identity for the loaded mountain.
    var currentDataset: MountainDataset?
    /// Timestamped operational state kept separate from `currentDataset`.
    var currentStatus: MountainStatus?
    /// Compatibility projection derived from dataset + status. New routing
    /// and matching code should take both explicit inputs during migration.
    ...
    private var datasetMemoryCache: [String: MountainDataset] = [:]
    private var statusMemoryCache: [String: MountainStatus] = [:]
    ...
    /// Importers need the same immutable mountain as the map, but must not
    /// navigate the visible map while a multi-file batch spans resorts. One
    /// task per resort coalesces concurrent files and publishes only to the
    /// shared dataset caches, never to `currentEntry` / `currentGraph`.
    private var importDatasetTasks: [String: Task<MountainDataset, Error>] = [:]
    private let overpassService = OverpassService()
    private let cacheVersion = "v6"
    /// Resort/network work is allowed to overlap, but only the newest exact
    /// request may publish. This also prevents a late legacy enrichment for an
    /// older request from replacing a newer canonical graph of the same resort.
    private var loadArbiter = ResortLoadArbiter()

    // MARK: - Public Load

    /// Loads the exact graph used to reconstruct activity runs without
    /// changing the resort currently displayed by the UI. This is the only
    /// cold-load path activity imports should use: cache-only lookup made run
    /// naming depend on whether the user had happened to open a resort first.
    func datasetForActivityImport(_ entry: ResortEntry) async throws -> MountainDataset {
        if currentEntry?.id == entry.id, let currentDataset, canReuseDataset(currentDataset, for: entry) {
            return currentDataset
        }
        if let cached = datasetMemoryCache[entry.id], canReuseDataset(cached, for: entry) {
            return cached
        }
        if let task = importDatasetTasks[entry.id] {
            return try await task.value
        }

        let task = Task { @MainActor [weak self] () throws -> MountainDataset in
            guard let self else { throw ResortLoadError.snapshotUnavailable }
            let (loadedDataset, _) = try await self.loadOrBuildGraph(entry: entry)
            if loadedDataset.source == .canonicalServer {
                self.datasetMemoryCache[entry.id] = loadedDataset
                self.graphMemoryCache[entry.id] = loadedDataset.graph
                return loadedDataset
            }

            let enriched = await GraphEnricher.enrich(
                loadedDataset.graph,
                resortId: entry.id
            )
            let delivered = loadedDataset.applyingLegacyEnrichment(enriched)
            self.datasetMemoryCache[entry.id] = delivered
            self.graphMemoryCache[entry.id] = enriched
            return delivered
        }
        importDatasetTasks[entry.id] = task
        defer { importDatasetTasks.removeValue(forKey: entry.id) }
        return try await task.value
    }
    ...
    func loadResort(
        ...
        datasetVersionOverride: String? = nil,
        forceOperationalRefresh: Bool = false
    ) async {
        let requestedSnapshot = snapshotOverride ?? SupabaseManager.shared.resolvedPinnedSnapshotDate(for: entry)
        let requestKey = ResortLoadRequestKey(
            resortID: entry.id,
            snapshotOverride: requestedSnapshot,
            manifestVersionOverride: manifestVersionOverride,
            datasetVersionOverride: datasetVersionOverride,
            forceOperationalRefresh: forceOperationalRefresh
        )
        guard let loadTicket = loadArbiter.begin(requestKey) else {
            // A caller asking for the same immutable mountain must await the
            // owner rather than returning early and observing half-loaded
            // state. A different request supersedes the owner and releases
            // this joiner as well; its caller's identity guard will then fail
            // safely instead of routing on the wrong graph.
            while loadArbiter.activeKey == requestKey {
        ...
        isLoading = true
        errorMessage = nil
        let requiresExactReplacement = currentEntry?.id != entry.id
            || (currentDataset?.source != .canonicalServer && currentSnapshotDate != requestedSnapshot)
            || (manifestVersionOverride != nil
                && manifestVersionOverride != currentManifestVersion)
            || (datasetVersionOverride != nil
                && datasetVersionOverride != currentDataset?.version.identifier)
        currentEntry = entry
        if requiresExactReplacement {
            // Do not display or expose an older resort/dataset while an exact
            // meet-version request is outstanding.
            currentGraph = nil
            currentDataset = nil
            currentStatus = nil
            currentResort = nil
           ...
           target != currentManifestVersion
            ...
            do {
                let result = try await CanonicalGraphFetcher.shared.fetch(
                    ...
                    datasetVersionIdentifier: datasetVersionOverride
                )
                guard loadArbiter.isLatest(loadTicket) else { return }
                if let datasetVersionOverride,
                   result.dataset.version.identifier != datasetVersionOverride {
                    throw ResortLoadError.datasetVersionMismatch
                }
                let stats = result.dataset.graph.makeResortStats(
                    resortId: entry.id,
                    snapshotDate: result.dataset.snapshotDate ?? ""
                )
                graphMemoryCache[entry.id] = result.graph
                datasetMemoryCache[entry.id] = result.dataset
                if let status = result.status {
                    statusMemoryCache[entry.id] = status
                } else {
                    statusMemoryCache.removeValue(forKey: entry.id)
                }
                statsMemoryCache[entry.id] = stats
                manifestVersionMemoryCache[entry.id] = result.dataset.version.manifestVersion
                currentEntry = entry
                currentResort = nil
                currentDataset = result.dataset
                currentStatus = result.status
                currentGraph = result.graph
                currentSnapshotDate = result.dataset.snapshotDate
                currentManifestVersion = result.dataset.version.manifestVersion
                currentSnapshotStats = stats
                errorMessage = nil
                isLoading = false
                loadArbiter.finish(loadTicket)
                return
            } catch {
                guard loadArbiter.isLatest(loadTicket) else { return }
                AppLog.graph.error("manifestVersionOverride fetch failed for \(entry.id) v\(target): \(error.localizedDescription)")
                errorMessage = "Required mountain version unavailable: \(error.localizedDescription)"
                isLoading = false
                loadArbiter.finish(loadTicket)
                return
            }
        ...
        // Foreground operational refresh is deliberately narrower than a full
        // fallback load. A temporary status/network failure must never replace
        // a verified canonical dataset with a legacy compatibility snapshot.
        if forceOperationalRefresh,
           currentDataset?.source == .canonicalServer,
           currentDataset?.resortID == entry.id {
            do {
                ...
                statsMemoryCache[entry.id] = stats
                if let manifest = result.dataset.version.manifestVersion {
                    manifestVersionMemoryCache[entry.id] = manifest
                }
                currentDataset = result.dataset
                currentStatus = result.status
                currentGraph = result.graph
                ...
                guard loadArbiter.isLatest(loadTicket) else { return }
                // If the old sidecar has expired, remove its projection from
                // routing. Keep the verified topology and expose the stale/nil
                // status to the global warning banner.
                if let dataset = currentDataset,
                   currentStatus?.isUsable() != true {
                    currentGraph = dataset.graph
                    graphMemoryCache[entry.id] = dataset.graph
                }
                AppLog.graph.info(
                    "Operational status refresh failed for \(entry.id): \(error.localizedDescription)"
                )
                isLoading = false
                loadArbiter.finish(loadTicket)
                return
    ...
    private func loadResortDefault(
        _ entry: ResortEntry,
        snapshotOverride: String? = nil,
        forceOperationalRefresh: Bool = false,
        loadTicket: ResortLoadArbiter.Ticket
    ) async {
        ...
        let canReuseMemory = datasetMemoryCache[entry.id].map {
            canReuseDataset($0, for: entry, snapshotOverride: snapshotOverride)
        } ?? false
        let bypassCaches = forceOperationalRefresh || !canReuseMemory

        if !bypassCaches, let cached = memoryCache[entry.id] {
            guard loadArbiter.isLatest(loadTicket) else { return }
            let cachedManifestVersion = manifestVersionMemoryCache[entry.id]
            let cachedDataset = datasetMemoryCache[entry.id]
            let cachedStatus = statusMemoryCache[entry.id]
            let displayedGraph = cachedDataset?.source == .canonicalServer
                ? cachedDataset?.routingGraph(applying: cachedStatus)
                : graphMemoryCache[entry.id]
            currentEntry = entry
            currentResort = cached
            currentGraph = displayedGraph
            currentDataset = cachedDataset
            currentStatus = cachedStatus
            currentSnapshotDate = cachedDataset?.snapshotDate
        ...
        if !bypassCaches, let cachedGraph = graphMemoryCache[entry.id] {
            // Prefer the immutable dataset's own stamp. Older in-memory
            // entries can still fall back to legacy graph-cache metadata.
            let snapshotDate: String?
            if let dataset = datasetMemoryCache[entry.id] {
                snapshotDate = dataset.snapshotDate
            } else {
                snapshotDate = await MountainRepository.shared
                    .loadLegacy(resortID: entry.id)?.dataset.snapshotDate
            }
        ...
        do {
            ...
            let (loadedDataset, loadedStatus) = try await loadOrBuildGraph(
                entry: entry,
                snapshotOverride: snapshotOverride
            )
            let graph = loadedDataset.routingGraph(applying: loadedStatus)
            let snapshotDate = loadedDataset.snapshotDate ?? ""
            let loadedFromCanonical = loadedDataset.source == .canonicalServer
            let canonicalManifestVersion = loadedDataset.version.manifestVersion
            ...
            let deliveredDataset: MountainDataset
            if loadedFromCanonical {
                deliveredDataset = loadedDataset
            } else {
                deliveredDataset = loadedDataset.applyingLegacyEnrichment(enrichedGraph)
            }
            guard loadArbiter.isLatest(loadTicket) else { return }

            // ── Deliver graph IMMEDIATELY so Location tester + routing work ──
            graphMemoryCache[entry.id] = enrichedGraph
            datasetMemoryCache[entry.id] = deliveredDataset
            if let loadedStatus {
                statusMemoryCache[entry.id] = loadedStatus
            } else {
                statusMemoryCache.removeValue(forKey: entry.id)
            }
            if let mv = canonicalManifestVersion {
            ...
            currentResort = memoryCache[entry.id]
            currentDataset = deliveredDataset
            ...
            // Only the ticket that still owns the latest generation may clear
            // loading. An older request finishing here must not hide the newer
            // request's spinner or publish its graph.
            isLoading = false
            loadArbiter.finish(loadTicket)
            ...
            Task { [weak self] in
                ...
                guard let self, self.loadArbiter.isLatest(loadTicket) else {
                    AppLog.graph.debug("Discarded stale background enrichment for \(resortId)")
                    return
                }
                self.graphMemoryCache[resortId] = enrichedGraph
                guard self.currentEntry?.id == resortId else { return }
            ...
            guard loadArbiter.isLatest(loadTicket) else {
                AppLog.graph.debug("Suppressed stale error for \(entry.id) — a newer load owns the UI")
                return
            }
            errorMessage = "Failed to load \(entry.name): \(error.localizedDescription)"
            // Make sure we don't leave a partially-loaded graph dangling.
            currentGraph = nil
            currentDataset = nil
            currentStatus = nil
            currentResort = nil
    ...
    private func canReuseDataset(_ dataset: MountainDataset, for entry: ResortEntry,
                                 snapshotOverride: String? = nil) -> Bool {
        guard dataset.resortID == entry.id else { return false }
        if dataset.source == .canonicalServer { return true }
        return LegacySnapshotCachePolicy.matches(dataset,
            resortID: entry.id,
            snapshotDate: snapshotOverride ?? SupabaseManager.shared.resolvedPinnedSnapshotDate(for: entry),
            graphVersion: MountainRepository.expectedLegacyVersion)
    }

    // MARK: - Graph Loading Pipeline

    /// Tries: canonical pipeline → exact disk cache → frozen server snapshot.
    /// It never creates routing topology from a device-specific live Overpass
    /// response. Disk-cache load and snapshot
    ...
    private func loadOrBuildGraph(
        entry: ResortEntry,
        snapshotOverride: String? = nil
    ) async throws -> (MountainDataset, MountainStatus?) {
        let phaseStart = Date()
        let phase: (String) -> Void = { label in
        ...
        //
        // If the canonical fetch fails, only a frozen legacy snapshot/cache
        // may be used as a compatibility preview.
        if CanonicalGraphFetcher.shared.isEnabled(for: entry.id) {
            phase("canonical:start")
            do {
                let result = try await CanonicalGraphFetcher.shared.fetch(resortId: entry.id)
                phase("canonical:fetched")
                return (result.dataset, result.status)
            } catch {
                AppLog.graph.error("Canonical fetch for \(entry.id) failed: \(error.localizedDescription) — checking frozen compatibility data")
            }
        }
        ...
        let requestedSnapshot = snapshotOverride ?? SupabaseManager.shared.resolvedPinnedSnapshotDate(for: entry)
        let cacheTask = Task { await MountainRepository.shared.loadLegacy(resortID: entry.id) }
        let snapshotTask = Task { await fetchFromSnapshotWithRetry(entry: entry, snapshotOverride: requestedSnapshot) }
        ...
        if let cached,
           canReuseDataset(cached.dataset, for: entry, snapshotOverride: requestedSnapshot) {
        ...
        // An old snapshot may contain empty or wrong-mountain geometry. Never
        // resurrect it after a failed request for the corrected immutable pin.
    ...
    /// resorts: a single invocation processes a server-selected batch of elevation coords, persists progress in a checkpoint blob, and
    ...
    private func driveSnapshotPipeline(
        ...
        // Freeze identity for the entire continuation sequence, even if server pins refresh.
        let pinnedDate = snapshotOverride ?? SupabaseManager.shared.resolvedPinnedSnapshotDate(for: entry)
        var progress = SnapshotBuildProgress()
        for iteration in 0..<SnapshotBuildProgress.maximumSteps {
            try Task.checkCancellation()
            let resp = try await invokeSnapshotFunction(
                entry: entry,
                isContinuation: iteration > 0,
                snapshotOverride: pinnedDate
            )
            guard resp.snapshot_date == pinnedDate else { throw URLError(.badServerResponse) }
            if resp.isReady {
                guard resp.osm_url != nil, resp.elevation_url != nil else { throw URLError(.badServerResponse) }
                return resp
            }
            guard resp.status == "elevation_pending", let prog = resp.elevation_progress else {
                throw URLError(.badServerResponse)
            }
            try progress.record(processed: prog.processed, total: prog.total)
            let pct = Int(Double(prog.processed) / Double(prog.total) * 100)
    ...
    private func invokeSnapshotFunction(
        ...
        // Supabase Edge Functions require both headers even when there is no
        // signed-in session (for example a cold import audit). The anon JWT
        // is the correct bearer fallback; authenticated users still send
        // their session token.
        request.setValue("Bearer \(token ?? SupabaseManager.anonKey)", forHTTPHeaderField: "Authorization")
    ...
    func clearCache(for id: String) {
        ...
        datasetMemoryCache.removeValue(forKey: id)
        statusMemoryCache.removeValue(forKey: id)
        manifestVersionMemoryCache.removeValue(forKey: id)
        let url = cacheURL(for: id)
        try? FileManager.default.removeItem(at: url)
        Task { await MountainRepository.shared.clearResort(id) }
    }

    // MARK: - Legacy Disk Cleanup

    /// The old-style `ResortData` disk path. Writes are gone (the current
    /// pipeline caches via `MountainRepository`), but `clearCache(for:)` still
    ...
    private func makeLegacyDataset(
        graph: MountainGraph,
        snapshotDate: String,
        ...
        MountainDataset(
            ...
            version: MountainDatasetVersion(
                manifestVersion: nil,
                graphVersion: MountainRepository.expectedLegacyVersion,
                contentSHA256: Data(graph.fingerprint.utf8).sha256Hex
            ),
            snapshotDate: snapshotDate,
            source: source,
            graph: graph
