...
nonisolated struct TrailMatcher {
    ...
    // A pause longer than this between consecutive fixes is a hard
    // segment boundary — paused recording, a lift with the watch off,
    // a lunch break. Without this the classifier bridges the gap and
    // two real runs collapse into one blob that then blows past the
    // run-duration cap and gets discarded entirely (the "runs falling
    // through because of large time gaps" report).
    private let maxIntraRunGapSeconds: TimeInterval = 90
    ...
    func segmentTrack(_ points: [GPXTrackPoint]) -> [SegmentedRun] {
        ...
        for i in 0..<points.count {
            // Hard split on a long time gap before anything else — a
            // pause means the previous run ended; never bridge it.
            if i > 0,
               let prev = points[i - 1].timestamp,
               let cur = points[i].timestamp,
               cur.timeIntervalSince(prev) > maxIntraRunGapSeconds {
                if currentGroup.count >= 2, let existing = currentIsLift {
        ...
        if let finalType = currentIsLift, currentGroup.count >= 2 {
