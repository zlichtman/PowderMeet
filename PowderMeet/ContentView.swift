...
struct ContentView: View {
    ...
    /// Inject the coordinator for previews and visual integration tests while
    /// keeping the same app views. Normal app launches always start services.
    @MainActor init(coordinator: ContentCoordinator? = nil, startsServices: Bool = true) {
        _coordinator = State(initialValue: coordinator ?? ContentCoordinator())
    ...
    @State private var openProfileActivity = false
    ...
    /// 60s clock that ticks the map friend-staleness refresh. Held in @State so
    /// it's created ONCE — constructing `Timer.publish(...).autoconnect()`
    /// inline in `.onReceive` rebuilt (and reset) the publisher on every body
    /// pass, so the 60s countdown almost never elapsed and the staleness
    /// refresh effectively never fired.
    @State private var mapFriendClock = Timer.publish(every: 60, on: .main, in: .common).autoconnect()
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ...
            if resortManager.currentDataset?.source == .canonicalServer,
               resortManager.currentStatus?.operatingMode == .offSeason {
                HStack(spacing: 8) {
                    ...
                    Text("MOUNTAIN OFF SEASON — LIVE ROUTING PAUSED")
                        .hudType(.label)
                        .tracking(0.7)
                    Spacer(minLength: 0)
                }
                ...
                .padding(.vertical, 7)
                .background(HUDTheme.secondaryText.opacity(0.08))
                .accessibilityLabel("Mountain off season. Live ski routing is paused.")
            } else if resortManager.currentDataset?.source == .canonicalServer,
                      resortManager.currentStatus?.isUsable() != true {
                HStack(spacing: 8) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .font(.system(size: 10, weight: .bold))
                    Text("LIVE TRAIL STATUS UNAVAILABLE — VERIFY CLOSURES")
                        .hudType(.label)
                        .tracking(0.7)
                    Spacer(minLength: 0)
                }
                ...
                .accessibilityLabel(
                    "Live trail status unavailable. Verify trail and lift closures with the resort."
                )
            }
            ...
            ZStack {
                ...
                VStack(spacing: 0) {
                    ...
                    if let session = coordinator.activeMeetSession {
                        CompactRouteSummary(
                            ...
                            friendSignalQuality: coordinator.friendQualityStore.qualities[
                                session.friendProfile.id
                            ],
                            friendLocation: coordinator.realtimeLocation?.friendLocations[
                                session.friendProfile.id
                            ]
                        ) {
                ...
                // Stacked-opacity tabs stay in the view tree even when hidden,
                // so without this VoiceOver would focus controls on the two
                // off-screen tabs. Hide the inactive ones from the a11y tree.
                ...
                ProfileView(
                    testMyNodeId: $coordinator.testMyNodeId,
                    openActivity: $openProfileActivity,
                    onPreviewRoute: { myNodeID, partnerNodeID in
                        Task {
                            if await coordinator.previewRouteRehearsal(
                                myNodeID: myNodeID,
        ...
        .sheet(isPresented: $showDestinationPicker) {
            if let dataset = resortManager.currentDataset,
               let graph = resortManager.currentGraph {
                DestinationRouteSheet(
                    ...
                    let succeeded = await coordinator.previewLandmarkRoute(to: point)
                    if succeeded { selectedTrailEdgeId = nil }
                    return succeeded
                }
            } else {
                ProgressView()
        ...
        .onReceive(mapFriendClock) { _ in
            guard coordinator.selectedEntry != nil else { return }
            if selectedTab == 0 {
                coordinator.mapFriendLayerClock &+= 1
            }
            coordinator.refreshOperationalStatusIfNeeded()
        }
        .onReceive(NotificationCenter.default.publisher(
            for: .powderMeetNetworkBecameReachable
        )) { _ in
            Task { await coordinator.meetRequestService.flushPendingTerminations() }
        }
        .onChange(of: coordinator.activeMeetSession?.id) { _, newId in
            ...
            coordinator.meetup.updateFriendRouteProgress()
        ...
        .onChange(of: activeFriendProgressKey) { _, _ in
            coordinator.meetup.updateFriendRouteProgress()
        }
    ...
    /// Stable change key for the active partner only. Coordinates and capture
    /// time are included so every accepted network update advances their
    /// independent route tracker without observing unrelated friends.
    private var activeFriendProgressKey: String? {
        guard let session = coordinator.activeMeetSession,
              let location = coordinator.realtimeLocation?.friendLocations[
                  session.friendProfile.id
              ] else { return nil }
    ...
    // resort) we render a flat brand-coloured splash instead of the
    // full Mapbox view — without this gate the Mapbox screen renders
    // for one tick on every fresh app load before the resort picker
    // sheet animates in, which read as a glitch flash.
    @ViewBuilder
    private var mapScreen: some View {
        if resortManager.currentEntry != nil {
            mapboxScreen
        } else {
            preResortSplash
        }
    ...
    /// What the user sees on cold launch BEFORE picking a resort — and
    /// also underneath the resort picker sheet when they drag it down
    /// without selecting. Intentionally not a real map: rendering even
    /// the Mapbox view at this point produces a one-frame flash of an
    /// unstyled world view that reads as a glitch.
    @ViewBuilder
    private var preResortSplash: some View {
        ZStack {
            HUDTheme.mapBackground.ignoresSafeArea()
            VStack(spacing: 14) {
                Image(systemName: "snowflake")
                    .font(.system(size: 44, weight: .light))
                    .foregroundColor(HUDTheme.accent.opacity(0.55))
    ...
    private var mapboxScreen: some View {
        ResortMapScreen(
            ...
            isLocalGuidancePaused: coordinator.activeMeetSession?.guidanceStatus != nil,
            activeRouteTracker: coordinator.activeMeetSession?.routeTracker,
            activeFriendRouteTracker: coordinator.activeMeetSession?.friendRouteTracker,
            ...
            friendSignalQualities: coordinator.friendQualityStore.qualities,
            showsDestinationRouting: resortManager.currentDataset != nil,
