...
final class MapBridge {
    ...
    /// True after the Mapbox style hits its retry ceiling without
    /// loading. Drives a "MAP UNAVAILABLE — check connection" overlay
    /// in `ResortMapScreen`. Reset to false on any successful style
    /// load (the retry happens automatically via `retryStyleLoad`,
    /// so the overlay self-dismisses when network returns).
    var styleLoadFailed: Bool = false
}
...
struct MountainMapInputs {
    ...
    var routeAInitialFraction: Double = 0
    var routeBInitialFraction: Double = 0
    var meetingNode: GraphNode? = nil
    var meetingDisplayName: String? = nil
    var rendezvousPoint: RendezvousPoint? = nil
    var routePresentationPurpose: RoutePresentationPurpose = .meetup
    ...
    /// On-mountain amenities (restaurants, huts, rest stops) fetched
    /// as a side channel by `ResortAmenitiesService` — never graph-
    /// derived, so the deterministic graph fingerprint is untouched.
    var amenities: [PointOfInterest] = []
}
...
struct MountainMapView: UIViewRepresentable {
    ...
    var friendSignalQualities: [UUID: FriendSignalQuality] = [:]
    /// On-mountain amenities; defaulted so existing call sites compile
    /// unchanged. Fed from `ResortMapScreen`'s amenity fetch.
    var amenities: [PointOfInterest] = []
    ...
    enum LayerID {
        ...
        static let trailCasingPalette = "trail-casing-palette-layer"
        static let trails             = "trails-layer"
        static let liftGlow           = "lift-glow-layer"
        static let lifts              = "lifts-layer"
        static let liftShimmer        = "lift-shimmer-layer"
        ...
        static let closedTerrainCasing = "closed-terrain-casing-layer"
        static let closedTerrain       = "closed-terrain-layer"
        static let closedTerrainLabels = "closed-terrain-labels-layer"
        ...
        static let meetingLabel       = "meeting-label-layer"
        ...
        static let overviewPOILabels  = "overview-poi-labels-layer"
        static let poiIcons           = "poi-icons-layer"
        static let poiLabels          = "poi-labels-layer"
        static let amenityIcons       = "amenity-icons-layer"
        static let amenityLabels      = "amenity-labels-layer"
    ...
    func makeUIView(context: Context) -> MapboxMaps.MapView {
        // Photo-real terrain makes the mountain legible at a glance: bowls,
        // tree line, ridges, and the valley between resort pods are visible
        // before the operational overlay is read. Our canonical graph remains
        // the source of truth for trails and routing; imagery is presentation.
        let options = MapInitOptions(
            cameraOptions: initialCamera(),
            styleURI: Self.operationalStyleURI,
            locationDataModel: LocationDataModel(
                location: context.coordinator.mapLocations.eraseToAnyPublisher()
            )
        )
        ...
        // Capture the coordinator WEAKLY in every map observer. The
        // coordinator strongly holds `mapView`, whose mapboxMap holds these
        // observer closures — a strong capture here is a retain cycle that
        // outlives the SwiftUI view (no deinit ever ran). Weak capture +
        // the deinit above lets the whole map stack deallocate on teardown.
        let coordinator = context.coordinator

        mapView.mapboxMap.onStyleLoaded.observeNext { [weak coordinator] _ in
            coordinator?.onStyleLoaded()
        }
        ...
        mapView.mapboxMap.onCameraChanged.observe { [weak coordinator] _ in
            Task { @MainActor in
                guard let coordinator,
                      let bridge = coordinator.inputs.mapBridge,
                      let mv = coordinator.mapView else { return }
    ...
    func updateUIView(_ mapView: MapboxMaps.MapView, context: Context) {
        ...
        coord.inputs = MountainMapInputs(
            ...
            routeA: routeA,
            routeB: routeB,
            routeAInitialFraction: routeAInitialFraction,
            routeBInitialFraction: routeBInitialFraction,
            meetingNode: meetingNode,
            meetingDisplayName: meetingDisplayName,
            rendezvousPoint: rendezvousPoint,
            routePresentationPurpose: routePresentationPurpose,
        ...
        // Active meetup acceptance: do not move the camera automatically.
        // The post-accept viewport stays where the user left it — every
        ...
        // careful user can find it. Explicit SHOW ROUTE / VIEW ON MAP taps
        // are handled separately from validated route geometry. Only the deactivation side touches
    ...
    /// One style URI for initial load and retry. Keeping these in sync also
    /// matches MapboxOfflineCache, so a transient style error cannot silently
    /// switch the map back to the flatter topographic presentation.
    private static let operationalStyleURI =
        StyleURI(rawValue: "mapbox://styles/mapbox/satellite-streets-v12")
        ?? .satelliteStreets
    ...
    final class Coordinator {
        ...
        // The app owns authorization and ski-session GPS lifetime. Supplying
        // this model at map creation prevents Mapbox creating a second Core
        // Location provider or requesting permission merely to display a map.
        let mapLocations = CurrentValueSubject<[MapboxMaps.Location], Never>([])
        ...
        var snowEmitterNear: CAEmitterLayer?
        ...
        var activeRouteStyleApplied = false
        ...
        /// Break every retain path when SwiftUI drops this coordinator (map
        /// tab torn down on sign-out / account deletion). Without this the
        /// Coordinator, the Mapbox MapView + its Metal surface, the ~40 style
        /// layers, and every runloop-registered CADisplayLink leaked for the
        /// app's lifetime and compounded on each teardown. `cancelBag` holds
        /// the map observer subscriptions — clearing it (plus the now-weak
        /// closure captures in makeUIView) severs the observer -> coordinator
        /// cycle; `pauseAnimations` invalidates the gondola/beam/friend
        /// display links and route/meeting timers; the bloom timer + snow
        /// emitters aren't covered by pause, so stop them here too.
        deinit {
            cancelBag.removeAll()
            pauseAnimations()
            arrivalBloomTimer?.invalidate()
            arrivalBloomTimer = nil
            snowEmitter?.removeFromSuperlayer()
            snowEmitter = nil
            snowEmitterNear?.removeFromSuperlayer()
            snowEmitterNear = nil
        }

        func retryStyleLoad() {
            guard !styleLoaded, let mapView else { return }
            // Exhausted the retry ladder — surface to the user via
            // the bridge so ResortMapScreen renders an actionable
            // "check connection" overlay. We still keep retrying on
            // every subsequent onMapLoadingError event (Mapbox fires
            // those when the user pans into uncached tiles), so the
            // moment the network returns the next retry succeeds and
            // `onStyleLoaded` flips the bridge flag back to false.
            guard styleRetryCount < maxStyleRetries else {
                inputs.mapBridge?.styleLoadFailed = true
                return
            }
            styleRetryCount += 1
            ...
            Task { @MainActor in
                ...
                mapView.mapboxMap.loadStyle(MountainMapView.operationalStyleURI)
            }
        }
        ...
        func onStyleLoaded() {
            ...
            styleLoaded = true
            // Recovery path: clear the user-visible failure flag and
            // reset the retry counter so a future failure starts fresh.
            inputs.mapBridge?.styleLoadFailed = false
            styleRetryCount = 0
            ...
            // Any puck consumers receive the same app-owned fixes through
            // mapLocations. The map never starts a separate GPS session.
        ...
        func updateDataLayers() {
            ...
            if trailStateChanged {
                ...
                if let graph = inputs.graph {
                    // Build every graph-derived source OFF the main thread.
                    // For a large resort this is a multi-hundred-ms pass
                    // (trail/lift/traverse/poi/temperature feature
                    // construction + Chaikin smoothing + the
                    // JSONSerialization→GeoJSONObject decode each source
                    // needs). Doing it synchronously here stalled the
                    // load/switch frame — the "weird lag." Mirror the
                    // established off-main pattern (`TreeLayerBuilder.
                    // populate`): build + decode on a userInitiated queue,
                    // apply on main. A fingerprint guard drops a stale
                    // build if a newer graph arrived meanwhile, so the
                    // single-source-update + hash-gate invariant holds.
                    let fp = graph.fingerprint
                    let showDebug = inputs.showDebugLayers
                    let amenities = inputs.amenities
                    let tempC = inputs.temperatureC
                    let entry = inputs.resortEntry
                    let snowy = inputs.snowfallCmPerHour > 0.5
                    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                        ...
                        let baseEle = graph.nodes.values
                            ...
                            .map(\.elevation)
                            .min() ?? 2000
                        var built: [(String, GeoJSONObject)] = []
                        func add(_ id: String, _ dict: [String: Any]) {
                            if let obj = Coordinator.decodeGeoJSON(dict) {
                                built.append((id, obj))
                            }
                        }
                        add(SourceID.trails, GeoJSONBuilder.trailFeatures(from: graph))
                        add(SourceID.lifts, GeoJSONBuilder.liftFeatures(from: graph))
                        add(SourceID.closedTerrain, GeoJSONBuilder.closedTerrainFeatures(from: graph))
                        // Walk/connector edges — always on (subtle) so trails
                        // that route via traverses don't look disconnected.
                        add(SourceID.traverses, GeoJSONBuilder.traverseFeatures(from: graph))
                        if showDebug {
                            add(SourceID.deadEnds, GeoJSONBuilder.deadEndNodeFeatures(from: graph))
                            add(SourceID.phantomTrails, GeoJSONBuilder.phantomTrailFeatures(from: graph))
                        } else {
                            add(SourceID.deadEnds, GeoJSONBuilder.emptyFeatureCollection())
                            add(SourceID.phantomTrails, GeoJSONBuilder.emptyFeatureCollection())
                        }
                        add(SourceID.pois, GeoJSONBuilder.poiFeatures(from: graph))
                        // Amenities are a side channel (not graph-derived) but
                        // only matter once a resort is on screen — piggyback.
                        add(SourceID.amenities, GeoJSONBuilder.amenityFeatures(from: amenities))
                        add(SourceID.liftEndpoints, GeoJSONBuilder.liftEndpointFeatures(from: graph))
                        add(SourceID.temperature, GeoJSONBuilder.temperatureFeatures(
                            from: graph, baseTemperatureC: tempC, baseElevationM: baseEle))

                        DispatchQueue.main.async { [weak self] in
                            guard let self,
                                  let mapView = self.mapView,
                                  let map = mapView.mapboxMap,
                                  self.inputs.graph?.fingerprint == fp
                            else { return }
                            for (id, obj) in built {
                                map.updateGeoJSONSource(withId: id, geoJSON: obj)
                            }
                            // Static lift lines carry the hierarchy more clearly
                            // than decorative 10 Hz gondola dots.
                            self.stopGondolaAnimation()
                            if let entry {
                                TreeLayerBuilder.populate(map: map, entry: entry,
                                                          graph: graph, snowyHint: snowy)
                            }
                        }
                    }
                } else {
                    ...
                    updateSource(map, id: SourceID.amenities, data: emptyFC)
                    updateSource(map, id: SourceID.liftEndpoints, data: emptyFC)
                    updateSource(map, id: SourceID.temperature, data: emptyFC)
                    updateSource(map, id: SourceID.closedTerrain, data: emptyFC)
                    TreeLayerBuilder.clear(map: map)
                }
            }
            ...
            let nextRouteState = MapRouteLayerState(
                ...
                routeAInitialPermille: Int((inputs.routeAInitialFraction * 1_000).rounded()),
                routeBInitialPermille: Int((inputs.routeBInitialFraction * 1_000).rounded()),
                meetingNode: inputs.meetingNode.map {
                    MapRouteLayerState.MeetingNodeKey(
                        $0,
                        displayName: inputs.meetingDisplayName,
                        rendezvousKind: inputs.rendezvousPoint?.kind,
                        markerVerb: inputs.routePresentationPurpose.mapMarkerVerb
                    )
                }
            )

            if nextRouteState.routeAEdgeIds != lastRouteLayerState?.routeAEdgeIds
                || nextRouteState.routeAInitialPermille != lastRouteLayerState?.routeAInitialPermille {
                if let routeA = inputs.routeA, !routeA.isEmpty {
                    let gj = GeoJSONBuilder.routeFeatures(
                        ...
                        graph: inputs.graph,
                        initialEdgeFraction: inputs.routeAInitialFraction
                    )
            ...
            if nextRouteState.routeBEdgeIds != lastRouteLayerState?.routeBEdgeIds
                || nextRouteState.routeBInitialPermille != lastRouteLayerState?.routeBInitialPermille {
                if let routeB = inputs.routeB, !routeB.isEmpty {
                    let gj = GeoJSONBuilder.routeFeatures(
                        ...
                        graph: inputs.graph,
                        initialEdgeFraction: inputs.routeBInitialFraction
                    )
            ...
            if nextRouteState.meetingNode != lastRouteLayerState?.meetingNode {
                if let meetingNode = inputs.meetingNode {
                    let gj = GeoJSONBuilder.meetingPointFeature(
                        node: meetingNode,
                        displayName: inputs.meetingDisplayName,
                        rendezvousPoint: inputs.rendezvousPoint,
                        markerVerb: inputs.routePresentationPurpose.mapMarkerVerb
                    )
            ...
            if sunNeedsRebuild, let graph = inputs.graph {
                ...
                // Sun exposure re-groups every run + Chaikin-smooths +
                // runs the solar calc per edge — as heavy as the trail
                // build, and it fires on graph load too. Off-main it
                // the same way; the bucket/inputs/fingerprint guard
                // drops a stale pass if a newer one was scheduled.
                let lat = inputs.resortLatitude ?? 39.6
                let lon = inputs.resortLongitude
                let time = inputs.selectedTime
                let tempC = inputs.temperatureC
                let cloud = inputs.cloudCoverPercent
                let fp = graph.fingerprint
                let bucket = currentBucket
                let inHash = sunInputsHash
                DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                    guard self != nil else { return }
                    let dict = GeoJSONBuilder.sunExposureFeatures(
                        from: graph,
                        at: time,
                        resortLatitude: lat,
                        resortLongitude: lon,
                    ...
                    let obj = Coordinator.decodeGeoJSON(dict)
                    DispatchQueue.main.async { [weak self] in
                        guard let self, let obj,
                              let mapView = self.mapView,
                              let map = mapView.mapboxMap,
                              self.inputs.graph?.fingerprint == fp,
                              self.lastSunExposureBucket == bucket,
                              self.lastSunInputsHash == inHash
                        else { return }
                        map.updateGeoJSONSource(withId: SourceID.sunExposure, geoJSON: obj)
                    }
                }
            }
            ...
            if userHash != lastUserLocationHash {
                ...
                if let userLoc = inputs.userLocation {
                    mapLocations.send([MapboxMaps.Location(coordinate: userLoc)])
        ...
        /// Pure `[String:Any]` → `GeoJSONObject`. The expensive
        /// serialize-then-reparse round-trip, isolated so it can run
        /// off the main thread (no `self` / Mapbox / main-only state).
        /// Returns nil on malformed input — caller skips that source
        /// rather than blanking it.
        nonisolated static func decodeGeoJSON(_ data: [String: Any]) -> GeoJSONObject? {
            guard
                let jsonData = try? JSONSerialization.data(withJSONObject: data),
                let obj = try? JSONDecoder().decode(GeoJSONObject.self, from: jsonData)
            else { return nil }
            return obj
        }
