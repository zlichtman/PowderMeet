...
struct MountainMapView: UIViewRepresentable {
    ...
    final class Coordinator {
        ...
        var routeChaseTimer: Timer?
        var routeChasePhase: Int = 0
        var lastRouteChaseHasRoutes: Bool = false
                ...
                if let graph = inputs.graph {
                    ...
                    DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                        ...
                        DispatchQueue.main.async { [weak self] in
                            ...
                            // Kick the gondola animation when the graph has
                            // any open lifts; idempotent.
                            if graph.lifts.contains(where: { $0.attributes.isOpen }) {
                                self.startGondolaAnimation()
                            } else {
