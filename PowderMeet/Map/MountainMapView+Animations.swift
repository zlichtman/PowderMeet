...
extension MountainMapView.Coordinator {
    ...
    func updateSnowEmitter() {
        ...
        let hasSnow = inputs.snowfallCmPerHour > 0.1
        let hasWindDrift = !hasSnow && inputs.windSpeedKph > 15
        ...
        let paramsHash: Int = {
            ...
            h.combine(hasWindDrift ? 1 : 0)
            return h.finalize()
        }()

        if paramsHash == lastSnowParamsHash && snowEmitter != nil { return }
        if paramsHash == lastSnowParamsHash && !hasSnow && !hasWindDrift { return }
        lastSnowParamsHash = paramsHash

        if hasSnow || hasWindDrift {
            if snowEmitter == nil {
            ...
            let density = Float(sqrt(min(inputs.snowfallCmPerHour / 5.0, 1.0)))
            ...
            if hasSnow {
                if snowEmitterNear == nil {
                    ...
                    emitter.emitterCells = [cell]
                    mapView.layer.addSublayer(emitter)
                    snowEmitterNear = emitter
                }

                snowEmitterNear?.emitterPosition = CGPoint(x: mapView.bounds.midX, y: -20)
                snowEmitterNear?.emitterSize = CGSize(width: mapView.bounds.width * 1.8, height: 1)

                if let cell = snowEmitterNear?.emitterCells?.first {
                    cell.birthRate = (30 + density * 170) * 0.3
                    let nearVelocity = baseVelocity * 1.6 + windBoost
                    cell.velocity = nearVelocity
                    cell.velocityRange = nearVelocity * 0.6
                    cell.emissionLongitude = .pi + CGFloat(leanRadians)
                    let calm = 1.0 - min(inputs.windSpeedKph / 60.0, 1.0)
                    cell.emissionRange = .pi / 8 + CGFloat(calm) * (.pi / 8)
                }
    ...
    func checkAndAnimateRoutes() {
        ...
        // Route-chase (the marching-dash animation) is deferred until
        // AFTER `startRouteAnimation` finishes its trim reveal. Earlier
        // this fired immediately on routes-appearing, so during the
        // ~0.5–1 s line draw-in the user saw a partially-revealed line
        // with dashes ALREADY marching across the visible portion —
        // visually busy and unprofessional. Now the reveal plays
        // alone (clean trim), then the chase activates in its
        // completion handler (`startRouteAnimation` below) alongside
        // the meeting-pin pulse. When routes are cleared we still
        // stop the chase here immediately so it never lingers past
        // its data source. The "routes set without animation" path
        // (post-reveal updateDataLayers, no trim work) also starts
        // the chase since the line is already fully drawn.
        if !hasRoutes, lastRouteChaseHasRoutes {
            lastRouteChaseHasRoutes = false
            stopRouteChase()
        }
        ...
        if (routesChanged || forceReplay) && hasRoutes {
            ...
            setRouteTrim(1.0)
            // Line is fully drawn but no trim animation just finished
            // (e.g. updateDataLayers tick while the meet remains
            // active). Make sure the chase is running so the route
            // doesn't go visually inert.
            if !lastRouteChaseHasRoutes {
                lastRouteChaseHasRoutes = true
                startRouteChase()
            }
        }
    }
    ...
    private func startRouteAnimation() {
        animationTimer?.invalidate()
        meetingPulseTimer?.invalidate()
        // Stop any in-flight chase before the new reveal — the line is
        // about to redraw from trim=0, so dashes marching on the old
        // line geometry would briefly continue against the new path
        // and read as a smear. The chase restarts cleanly in the
        // reveal's completion handler.
        if lastRouteChaseHasRoutes {
            stopRouteChase()
            lastRouteChaseHasRoutes = false
        }
        ...
        animationTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 60.0, repeats: true) { [weak self] timer in
            ...
            if t >= 1.0 {
                ...
                // Reveal complete — pulse the meeting pin AND start
                // the marching-dash chase together. Doing both at the
                // same beat reads as "route is now live" rather than
                // "reveal kind of finished, then something else
                // happened." `lastRouteChaseHasRoutes` is the gate
                // `checkAndAnimateRoutes` uses to know whether to
                // start/stop the chase on subsequent updateDataLayers
                // ticks; flip it true now so the chase keeps running.
                self.startMeetingPulse()
                if !self.lastRouteChaseHasRoutes {
                    self.lastRouteChaseHasRoutes = true
                    self.startRouteChase()
                }
            }
        }
    }
    ...
    func startRouteChase() {
        guard let map = mapView?.mapboxMap else { return }
        if reduceMotionEnabled {
            let stat: [Double] = [3, 9]
            try? map.setLayerProperty(for: MountainMapView.LayerID.routeA, property: "line-dasharray", value: stat)
            try? map.setLayerProperty(for: MountainMapView.LayerID.routeB, property: "line-dasharray", value: stat)
            return
        }
        routeChaseTimer?.invalidate()
        routeChasePhase = 0
        applyRouteChaseDash()
        routeChaseTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 12.0, repeats: true) { [weak self] timer in
            guard let self else { timer.invalidate(); return }
            self.routeChasePhase = (self.routeChasePhase + 1) % 12
            self.applyRouteChaseDash()
        }
    }
    ...
    private func applyRouteChaseDash() {
        guard let map = mapView?.mapboxMap else { return }
        let dash = routeChaseDashArray(phase: routeChasePhase)
        try? map.setLayerProperty(for: MountainMapView.LayerID.routeA, property: "line-dasharray", value: dash)
        try? map.setLayerProperty(for: MountainMapView.LayerID.routeB, property: "line-dasharray", value: dash)
    }

    private func routeChaseDashArray(phase: Int) -> [Double] {
        let p = ((phase % 12) + 12) % 12
        switch p {
        case 0: return [3, 9]
        case 1...8:
            return [0, Double(p), 3, Double(9 - p)]
        case 9: return [0, 9, 3, 0]
        case 10: return [2, 9, 1, 0]
        case 11: return [1, 9, 2, 0]
        default: return [3, 9]
        }
    }
