...
extension MountainMapView.Coordinator {
    ...
    func resumeAnimationsIfNeeded() {
        ...
        if activeRouteStyleApplied {
            applyActiveRouteStyle()
        }
    ...
    func checkAndAnimateRoutes() {
        ...
        let previousAEdgeIds = lastAnimatedRouteAEdgeIds
        let previousBEdgeIds = lastAnimatedRouteBEdgeIds
        let routesChanged = nextAEdgeIds != previousAEdgeIds
                         || nextBEdgeIds != previousBEdgeIds
        let shouldReplay = MapRouteAnimationPolicy.shouldReplay(
            previousA: previousAEdgeIds,
            previousB: previousBEdgeIds,
            nextA: nextAEdgeIds,
            nextB: nextBEdgeIds,
            forced: forceReplay
        )
        ...
        // Camera movement is reserved for the explicit user action that bumps
        // `routeAnimationTrigger`. Automatic solve/accept/reroute updates keep
        // the viewport untouched. The framing helper admits only route
        // geometry inside the current resort, never a possibly stale puck.
        if forceReplay && hasRoutes {
            frameExplicitRoutePreview()
        }

        // The reveal is finite; once complete, the corridor stays solid and
        // quiet. Prior builds ran a perpetual 12 Hz marching-dash timer here,
        // adding visual noise and background Mapbox writes for no navigation
        // gain. Clearing routes restores the same stable line property.
        if !hasRoutes, activeRouteStyleApplied {
            activeRouteStyleApplied = false
            clearActiveRouteStyle()
        }

        if shouldReplay && hasRoutes {
            startRouteAnimation()
            ...
            // active). Make sure the settled route style is applied.
            if !activeRouteStyleApplied {
                activeRouteStyleApplied = true
                applyActiveRouteStyle()
            }
        }
    }

    // The map's automatic overview-on-meetup-accept path was removed: every
    // implementation kept landing the camera on a stale or unprimed
    // location subject ("middle of nowhere"), and the user prefers
    // a no-camera-move policy on accept. An explicit SHOW ROUTE / VIEW ON MAP
    // trigger is different: it safely frames validated route geometry below.
    // Route lines still animate
    ...
    private func startRouteAnimation() {
        animationTimer?.invalidate()
        meetingPulseTimer?.invalidate()
        // Reset the settled route style before a fresh finite reveal.
        if activeRouteStyleApplied {
            clearActiveRouteStyle()
            activeRouteStyleApplied = false
        }
        ...
        lastRouteTrim = -1
        setRouteTrim(0)
        // Explicit preview framing already ran in `checkAndAnimateRoutes`.
        // Automatic route changes draw in place without touching the camera.
        ...
        animationTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 60.0, repeats: true) { [weak self] timer in
            ...
            if t >= 1.0 {
                ...
                // Reveal complete — settle into one calm, solid corridor and
                // pulse the destination once.
                self.startMeetingPulse()
                if !self.activeRouteStyleApplied {
                    self.activeRouteStyleApplied = true
                    self.applyActiveRouteStyle()
                }
            }
        }
    }

    private func frameExplicitRoutePreview() {
        guard let entry = inputs.resortEntry,
              let director = cinemaDirector else { return }
        let coordinates = ExplicitRoutePreviewFraming.coordinates(
            routes: [inputs.routeA ?? [], inputs.routeB ?? []],
            graph: inputs.graph,
            meetingNode: inputs.meetingNode,
            resortBounds: entry.bounds
        )
        guard coordinates.count >= 2 else { return }
        let bearing = entry.preferredBearing
            ?? mapView?.mapboxMap.cameraState.bearing
        director.frame(
            coordinates: coordinates,
            padding: UIEdgeInsets(top: 150, left: 48, bottom: 260, right: 48),
            duration: reduceMotionEnabled ? 0 : 0.7,
            bearing: bearing
        )
    }
    ...
    func applyActiveRouteStyle() {
        guard let map = mapView?.mapboxMap else { return }
        let dash = ActiveRouteLineStyle.dashArray
