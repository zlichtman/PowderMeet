...
final class CinemaDirector {
    ...
    /// In-flight resort-intro camera eases. `camera.ease(...)` returns a
    /// `Cancelable`, NOT a `BasicCameraAnimator` — the old `[BasicCameraAnimator]`
    /// array could never be populated, so `cancelActiveIntro()` iterated an
    /// empty list and a meetup accept mid-intro left the old resort's camera
    /// ease still playing ("middle of nowhere" drift). Collect the real
    /// cancelables so cancellation actually stops the camera.
    private var activeIntroCancelables: [Cancelable] = []
    ...
    func playResortIntro(landing: CameraOptions) async {
        ...
        activeIntroCancelables.append(mapView.camera.ease(to: stage1, duration: 0.5, curve: .easeOut, completion: nil))
        await sleep(0.5)
        guard introToken == resortIntroToken else { return }
        activeIntroCancelables.append(mapView.camera.ease(to: stage2, duration: 1.4, curve: .easeInOut, completion: nil))
        await sleep(1.4)
        guard introToken == resortIntroToken else { return }
        activeIntroCancelables.append(mapView.camera.ease(to: landing, duration: 1.1, curve: .easeOut, completion: nil))
        await sleep(1.1)
    }
    ...
    private func cancelActiveIntro() {
        for cancelable in activeIntroCancelables { cancelable.cancel() }
        activeIntroCancelables.removeAll()
    }
