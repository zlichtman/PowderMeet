...
enum TreeLayerBuilder {
    ...
    private static let elevationGridCellMeters: Double = 500
    private static let maximumElevationReferenceDistanceMeters: Double = 1_500
    ...
    static func generateFeatureCollection(entry: ResortEntry, graph: MountainGraph?, snowyHint: Bool) -> FeatureCollection {
        ...
        let elevationIndex = buildElevationGrid(
            graph: graph,
            bounds: bounds,
            latDegPerMeter: latDegPerMeter,
            lonDegPerMeter: lonDegPerMeter
        )
        let treeLineMeters = entry.treeLineMeters ?? defaultTreeLineMeters
        ...
        for _ in 0..<attempts {
            ...
            // The routing graph already carries sampled DEM elevation at every
            // node. Interpolate from nearby nodes so decorative trees respect
            // the mountain's alpine zone instead of appearing above tree line.
            // If a random point is too far from any graph evidence, omit it:
            // the satellite base still shows the real forest, while invented
            // symbols should stay close to terrain PowderMeet actually knows.
            guard let estimatedElevation = estimatedElevation(
                at: coord,
                grid: elevationIndex
            ), estimatedElevation <= treeLineMeters else { continue }
    ...
    private static func buildElevationGrid(
        ...
        var grid = ElevationGrid(
            cellMeters: elevationGridCellMeters,
        ...
        for node in graph.nodes.values where node.elevation.isFinite {
            let key = gridKey(
                for: node.coordinate,
                originLat: grid.originLat,
                originLon: grid.originLon,
                cellMeters: grid.cellMeters,
                latDegPerMeter: grid.latDegPerMeter,
                lonDegPerMeter: grid.lonDegPerMeter
            )
            grid.cells[key, default: []].append(node)
        }
        return grid
    }

    /// Inverse-distance interpolation over the three nearest graph nodes.
    /// Three anchors smooth steep edge-to-edge jumps while keeping generation
    /// bounded: the spatial grid limits every lookup to a 1.5 km neighborhood.
    private static func estimatedElevation(
        at coordinate: CLLocationCoordinate2D,
        grid: ElevationGrid
    ) -> Double? {
        let cellLatSpan = grid.cellMeters * grid.latDegPerMeter
        let cellLonSpan = grid.cellMeters * grid.lonDegPerMeter
        let centerI = Int(floor((coordinate.latitude - grid.originLat) / cellLatSpan))
        let centerJ = Int(floor((coordinate.longitude - grid.originLon) / cellLonSpan))
        let radius = Int(ceil(maximumElevationReferenceDistanceMeters / grid.cellMeters))
        let maximumDistanceSquared = maximumElevationReferenceDistanceMeters
            * maximumElevationReferenceDistanceMeters
        var nearest: [(distanceSquared: Double, elevation: Double)] = []
        func comesBefore(
            _ lhs: (distanceSquared: Double, elevation: Double),
            _ rhs: (distanceSquared: Double, elevation: Double)
        ) -> Bool {
        ...
        for di in -radius...radius {
            for dj in -radius...radius {
                let key = gridKey(i: centerI + di, j: centerJ + dj)
                guard let nodes = grid.cells[key] else { continue }
                for node in nodes {
                    let north = (node.coordinate.latitude - coordinate.latitude)
                        / grid.latDegPerMeter
                    let east = (node.coordinate.longitude - coordinate.longitude)
                        / grid.lonDegPerMeter
                    let distanceSquared = north * north + east * east
                    guard distanceSquared <= maximumDistanceSquared else { continue }
                    let candidate = (distanceSquared, node.elevation)
                    if nearest.count < 3 {
                        nearest.append(candidate)
                        nearest.sort(by: comesBefore)
                    } else if let farthest = nearest.last,
                              comesBefore(candidate, farthest) {
                        nearest[nearest.count - 1] = candidate
                        nearest.sort(by: comesBefore)
                    }
                }
            }
        ...
        for anchor in nearest {
            // A 25 m floor prevents a node-level sample from approaching
            // infinite weight while still making it the dominant anchor.
            let weight = 1 / max(25, anchor.distanceSquared.squareRoot())
            weightedElevation += anchor.elevation * weight
            totalWeight += weight
        }
        return totalWeight > 0 ? weightedElevation / totalWeight : nil
    }
    ...
    private static func gridKey(
        ...
        let i = Int(floor(
            (coordinate.latitude - originLat) / (cellMeters * latDegPerMeter)
        ))
        let j = Int(floor(
            (coordinate.longitude - originLon) / (cellMeters * lonDegPerMeter)
        ))
        return gridKey(i: i, j: j)
    }

    private static func gridKey(i: Int, j: Int) -> Int64 {
        (Int64(i) << 32) | (Int64(j) & 0xFFFFFFFF)
    }
