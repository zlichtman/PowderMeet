...
struct ResortMapScreen: View {
    ...
    /// On-mountain amenities for the current resort. Fetched as a
    /// side channel (see `ResortAmenitiesService`) keyed by the
    /// resort id so it's independent of the graph build path.
    @State private var amenities: [PointOfInterest] = []
    ...
    var isLocalGuidancePaused: Bool = false
    /// Local progress is intentionally separate from `meetingResult`: the
    /// result retains the accepted full path, while the map renders only the
    /// corridor that still lies ahead.
    var activeRouteTracker: RouteProgressTracker? = nil
    var activeFriendRouteTracker: RouteProgressTracker? = nil
    ...
    var showsDestinationRouting: Bool = false
    var onOpenDestinationRouting: (() -> Void)? = nil

    var body: some View {
        let localRoute = activeLocalRoute.visible(isGuidancePaused: isActiveMeetup && isLocalGuidancePaused)
        let friendRoute = activeFriendRoute
        ZStack(alignment: .bottom) {
            ...
            MountainMapView(
                ...
                routeA: localRoute.edges,
                routeB: friendRoute.edges,
                routeAInitialFraction: localRoute.initialEdgeFraction,
                routeBInitialFraction: friendRoute.initialEdgeFraction,
                meetingNode: meetingResult?.meetingNode,
                meetingDisplayName: meetingResult?.meetingDisplayName,
                rendezvousPoint: meetingResult?.rendezvousPoint,
                routePresentationPurpose: meetingResult?.presentationPurpose ?? .meetup,
                ...
                ghostPositions: MapGuidancePresentation.visibleProjections(
                    ghostPositions, localGuidancePaused: isActiveMeetup && isLocalGuidancePaused,
                    partnerID: meetupPartnerId
                ),
                ...
                VStack {
                    HStack {
                        Button {
                            ...
                            .frame(minHeight: 44)
                            .background(HUDTheme.cardBackground.opacity(0.94))
                            .clipShape(Capsule())
                            .overlay(
                                Capsule().stroke(HUDTheme.accent.opacity(0.55), lineWidth: 0.8)
                            )
                            .contentShape(Capsule())
                        }
                        .buttonStyle(.plain)
                        .accessibilityLabel("Choose a mountain destination")
            ...
            // Mapbox style failure: retry ladder exhausted without
            // loading. Could be ISP DNS hijack, captive portal, blocked
            // domain, or a Mapbox outage. The retry loop continues in
            // the background on every onMapLoadingError event, so this
            // overlay self-dismisses the moment a load succeeds.
            if mapBridge?.styleLoadFailed == true {
                VStack(spacing: 8) {
                    ...
                    Text("MAP UNAVAILABLE")
                        ...
                        .foregroundColor(HUDTheme.primaryText)
                        .tracking(1.5)
                    Text("Check your connection — retrying.")
                        .font(.system(size: 11))
                ...
                .padding(.vertical, 16)
                .background(HUDTheme.cardBackground.opacity(0.95))
                .clipShape(RoundedRectangle(cornerRadius: 10, style: .continuous))
                .overlay(
                    RoundedRectangle(cornerRadius: 10, style: .continuous)
                        .strokeBorder(HUDTheme.cardBorder, lineWidth: 1)
                )
                .padding(.bottom, 140)
                .zIndex(20)
        ...
        // Amenity fetch — keyed by resort id so it re-runs on every
        // resort switch and cancels the previous fetch automatically.
        // Cached on disk by the service; a cold cache resolves after
        // the Overpass round-trip and the map's amenity layer fills
        // in without blocking the graph render.
        .task(id: resortManager.currentEntry?.id) {
            guard let entry = resortManager.currentEntry else {
                amenities = []
                return
            }
            let fetched = await ResortAmenitiesService.shared.amenities(for: entry)
            // Guard against a late return after the user already
            // switched resorts again.
            if resortManager.currentEntry?.id == entry.id {
                amenities = fetched
            }
        }
    ...
    private var activeLocalRoute: ActiveMapRouteSlice {
        if isActiveMeetup, let tracker = activeRouteTracker {
            return .remaining(
                path: tracker.path,
        ...
        return ActiveMapRouteSlice(
            edges: meetingResult?.pathA ?? [],
            initialEdgeFraction: meetingResult?.initialEdgeFractionA ?? 0
        )
    }

    private var activeFriendRoute: ActiveMapRouteSlice {
        if isActiveMeetup, let tracker = activeFriendRouteTracker {
            return .remaining(
                path: tracker.path,
                currentEdgeIndex: tracker.currentEdgeIndex,
                currentEdgeFraction: tracker.currentEdgeFraction
        ...
        return ActiveMapRouteSlice(
            edges: meetingResult?.pathB ?? [],
            initialEdgeFraction: meetingResult?.initialEdgeFractionB ?? 0
        )
    }
...
struct ResortLoadingView: View {
    ...
    var body: some View {
        ...
        // `.task` (not `.onAppear { Task { … } }`) so the status-cycling loop
        // is cancelled when the loading view disappears. The old unstructured
        // Task kept sleeping + animating forever on a dead view, and every
        // resort switch spawned another immortal copy.
        .task {
