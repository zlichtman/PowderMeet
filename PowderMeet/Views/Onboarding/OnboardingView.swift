...
struct OnboardingView: View {
    ...
    /// Tracks whether iOS has recorded an answer to the push permission
    /// dialog yet. The notifications step (step 3) gates CONTINUE on
    /// this — we don't let the user finish onboarding before they've
    /// either granted or explicitly denied; staying on .notDetermined
    /// would mean tapping "GET STARTED" without registering a device
    /// token, which is the gap we're closing.
    @State private var notificationsAnswered = false

    private let totalSteps = 4

    var body: some View {
        ZStack {
            // Same ambient topo watermark as the auth landing so the
            // pre-account flow (sign in → onboarding) reads as one
            // continuous branded surface. Radial-cleared through the
            // center so it never sits behind step content or the CTA.
            ...
            ZStack {
                ...
                case 2:
                    OnboardingLocationStep()
                        .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)))
                case 3:
                    OnboardingNotificationsStep(onStatusChange: { status in
                        // .notDetermined is the only state we don't
                        // accept — granted, denied, provisional, and
                        // ephemeral all count as "iOS has an answer."
                        notificationsAnswered = (status != .notDetermined)
                    })
                    .transition(.asymmetric(insertion: .move(edge: .trailing), removal: .move(edge: .leading)))
                default:
                    EmptyView()
    ...
    private var continueEnabled: Bool {
        ...
        // The notifications step (step 3) gates CONTINUE on iOS having
        // recorded *some* answer — granted, denied, anything but
        // .notDetermined. Without the gate a fresh signup could finish
        // onboarding before the dialog resolves and hit the lift with
        // no device_tokens row.
        if step == 3 { return notificationsAnswered }
        return true
    }
    ...
    private func completeOnboarding() async {
        ...
        AppLog.supabase.debug("[Onboarding] Step 1: ensureProfileExists...")
        var profileReady = false
        for attempt in 1...3 {
            profileReady = await supabase.ensureProfileExists()
            if profileReady { break }
            AppLog.supabase.error("[Onboarding] ensureProfileExists attempt \(attempt) failed, retrying...")
            try? await Task.sleep(for: .milliseconds(500))
        }
        if !profileReady {
            AppLog.supabase.error("[Onboarding] Could not create profile after 3 attempts")
            saveError = "Failed to create your profile. Please check your connection and try again."
            isSaving = false
            return
        }
        AppLog.supabase.debug("[Onboarding] Step 1 done")
        ...
        if let avatarData {
            AppLog.supabase.debug("[Onboarding] Step 2: uploading avatar (\(avatarData.count) bytes)...")
            ...
            for attempt in 0..<2 {
                do {
                    ...
                    lastError = error
                    AppLog.supabase.error("[Onboarding] Avatar upload attempt \(attempt + 1) failed: \(error)")
                    if attempt == 0 {
        ...
        do {
            ...
            AppLog.supabase.error("[Onboarding] DB update failed: \(error)")
