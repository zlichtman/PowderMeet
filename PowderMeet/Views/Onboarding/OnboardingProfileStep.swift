...
struct OnboardingProfileStep: View {
    ...
    private func refreshPreferredSki() async {
        ...
        // Prefer the cached catalog; only fetch over the network if it's cold.
    ...
    /// A full-resolution library photo (12MP+) makes the orientation
    ...
    nonisolated private static func squareAvatarJPEG(from uiImage: UIImage) -> Data? {
        ...
        guard let cgCropped = normalized.cgImage?.cropping(to: cropRect) else { return nil }
