...
struct AuthView: View {
        ...
        ZStack {
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    .background(HUDTheme.inputBackground.opacity(0.65))
                    .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
                    .overlay(
                        RoundedRectangle(cornerRadius: 8, style: .continuous)
                            .stroke(HUDTheme.cardBorder.opacity(0.75), lineWidth: 0.75)
                    )
                    .padding(.bottom, 16)
                    ...
                    .padding(20)
                    .background(HUDTheme.inputBackground.opacity(0.72))
                    .clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
                    .overlay(
                        RoundedRectangle(cornerRadius: 12, style: .continuous)
                            .stroke(HUDTheme.cardBorder.opacity(0.9), lineWidth: 0.75)
                    )
    ...
    private func tabButton(title: String, isActive: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(title)
                .hudType(.label)
                // An inactive tab is still an available action. Keep it
                // visibly secondary without the disabled-looking low contrast
                // that made SIGN UP appear unavailable.
                .foregroundColor(isActive ? .white : HUDTheme.textSecondary)
                .tracking(1.5)
