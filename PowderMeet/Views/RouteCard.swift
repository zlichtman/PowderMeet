...
struct RouteCard: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Legacy/non-canonical route warning. New interactive solves are
            // strict; older saved previews remain visibly non-navigable.
            ...
            HStack(spacing: 10) {
                Image(systemName: "mappin.circle.fill")
                    .font(.system(size: 14))
                    // Gold meeting-pin token — matches CompactRouteSummary and
                    // the map pin. This used to hardcode accentGreen, so the
                    // meeting point rendered green here and gold everywhere else.
                ...
                VStack(alignment: .leading, spacing: 2) {
                    Text("MEETING POINT")
                        ...
                        .foregroundColor(HUDTheme.secondaryText)
                        .tracking(1)
                    if let name = result.meetingDisplayName
                        ?? result.rendezvousPoint?.displayName,
                       !name.isEmpty {
    ...
    private func fallbackPreviewPill(for attempt: SolveAttempt) -> some View {
        let copy: String = {
            ...
            case .forcedOpenNeighborSubstitution:
                return "PREVIEW · CLOSED TRAILS + NEAREST NODE"
            case .nonCanonicalDataset:   return "PREVIEW · MOUNTAIN DATA NOT VERIFIED"
            }
        }()
        HStack(spacing: 6) {
    ...
    private func routeColumn(label: String, color: Color, path: [GraphEdge], time: Double) -> some View {
        ...
        return VStack(spacing: 8) {
            ...
            // Distance
            statRow(icon: "arrow.left.and.right", value: UnitFormatter.distance(distance), accent: color)
