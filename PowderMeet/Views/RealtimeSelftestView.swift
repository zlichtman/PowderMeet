...
struct RealtimeSelftestView: View {
    ...
    var body: some View {
        // Custom HStack header (no NavigationStack) to keep the
        // HUDDoneButton out of the iOS Liquid Glass toolbar wrapper.
        VStack(spacing: 0) {
            HStack {
                Text("REALTIME SELFTEST")
                    .hudType(.bodyEmph)
        ...
        return HStack(alignment: .top, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                if case .pass(let msg) = res, !msg.isEmpty {
                    Text(msg).hudType(.label).foregroundStyle(.secondary)
                } else if case .fail(let msg) = res {
                    Text(msg).hudType(.label).foregroundStyle(.red)
            ...
            .padding(.vertical, 8)
            .background(HUDTheme.cardBackground, in: RoundedRectangle(cornerRadius: 6))
            .overlay(RoundedRectangle(cornerRadius: 6).stroke(HUDTheme.cardBorder, lineWidth: 1))
            .controlSize(.mini)
            .disabled(running)
        }
    }
    ...
    private func checkMonotonic() async -> Result {
        ...
        let fresh = RealtimeLocationService.FriendLocation(
            userId: userId, displayName: "T", resortId: "test", latitude: 0, longitude: 0,
            capturedAt: now, nearestNodeId: nil, accuracyMeters: nil
        )
        let stale = RealtimeLocationService.FriendLocation(
            userId: userId, displayName: "T", resortId: "test", latitude: 1, longitude: 1,
    ...
    private func checkStoreRemove() async -> Result {
        do {
            ...
            let loc = RealtimeLocationService.FriendLocation(
                userId: userId, displayName: "Test", resortId: "test", latitude: 39.6, longitude: -106.35,
                capturedAt: Date(), nearestNodeId: nil, accuracyMeters: 12
            )
            store.upsert(loc)
            guard store.loadAll(resortId: "test").contains(where: { $0.userId == userId }) else {
