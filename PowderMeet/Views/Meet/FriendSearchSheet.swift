...
struct FriendSearchSheet: View {
    ...
    var body: some View {
        ...
        // Don't let the keyboard heave the sheet (background included)
        // upward when the search field takes focus.
    ...
    private func searchResultRow(_ user: UserProfile) -> some View {
        ...
        return HStack(spacing: 12) {
            ...
            case .pendingSent:
                ...
                .buttonStyle(.plain)
                .accessibilityLabel("Cancel pending friend request to \(user.displayName)")
            ...
            case .none:
                if justSent {
                    ...
                    .buttonStyle(.plain)
                    .accessibilityLabel("Add \(user.displayName) as a friend")
                }

            case .blocked:
                // Blocked rows shouldn't normally appear in search
                // (the service filters them out), but show a neutral
                // marker for the edge case where a row arrives
                // through a path that bypasses the filter.
                Text("BLOCKED")
                    .hudType(.caption)
                    .foregroundColor(HUDTheme.secondaryText)
                    .tracking(1)
