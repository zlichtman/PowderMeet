...
enum RouteStepConsolidator {
    ...
    /// edges merge only across the shared identity, kind, continuity, label,
    /// and difficulty boundary used by written directions and the live HUD.
    ///
    ...
    /// data (e.g. tests). The fallback preserves explicit trail names and
    /// otherwise uses a generic kind label, which is never a merge identity.
    ///
    ...
    static func consolidate(
        ...
        for (i, edge) in path.enumerated() {
            ...
            if let previous = previousEdge, let lastName,
               RouteInstructionGrouping.canMerge(
                previous, edge, previousLabel: lastName, nextLabel: displayName
               ) {
                ...
                // Keep the immediate predecessor for directed continuity.
                previousEdge = edge
                continue
            }
    ...
    static func consolidatedIndex(
        ...
        for (i, edge) in path.enumerated() {
            ...
            let isMerged = previousEdge.map {
                RouteInstructionGrouping.canMerge(
                    $0, edge, previousLabel: lastName ?? "", nextLabel: name
                )
            } ?? false
            if !isMerged {
