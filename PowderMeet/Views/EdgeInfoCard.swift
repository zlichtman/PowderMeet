...
nonisolated enum TerrainAvailability: Equatable {
    case open
    case partiallyOpen(open: Int, total: Int)
    case closed

    init(edges: [GraphEdge]) {
        let total = edges.count
        let open = edges.filter(\.attributes.isOpen).count
        if total == 0 || open == total {
            self = .open
        } else if open == 0 {
            self = .closed
        } else {
            self = .partiallyOpen(open: open, total: total)
        }
    }
    ...
    var label: String {
        ...
        case .partiallyOpen(let open, let total): return "PARTIAL · \(open)/\(total)"
...
struct EdgeInfoCard: View {
    ...
    private var operationalEdges: [GraphEdge] {
        trailGroup.isEmpty ? [edge] : trailGroup
    }
    private var availability: TerrainAvailability {
        TerrainAvailability(edges: operationalEdges)
    }
    ...
    private var availabilityColor: Color {
        switch availability {
        case .open: return HUDTheme.accentGreen
        case .partiallyOpen: return HUDTheme.accentAmber
        case .closed: return HUDTheme.accentRed
        }
    }
    ...
    private var dismissButton: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("Dismiss trail info")
    }
    ...
    private var trailStats: some View {
        HStack(spacing: 14) {
            statLabel("LENGTH", value: UnitFormatter.distance(totalLength))
    ...
    private var liftStats: some View {
        HStack(spacing: 14) {
            ...
            statLabel("LENGTH", value: UnitFormatter.distance(edge.attributes.lengthMeters))
