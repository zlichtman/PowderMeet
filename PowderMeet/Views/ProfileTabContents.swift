...
/// Smaller, line-rule-free label one tier below `sectionHeader`.
    ...
    /// only the gating (`maxRunDifficulty`, gradient/snow penalties)
    ...
    private func updateSkillLevel(_ key: String) {
        guard let current = profile, current.skillLevel != key else { return }
        Task {
            do {
        ...
        VStack(spacing: 8) {
            ...
            Button { showThemePicker = true } label: {
                displayRow(
                    ...
                    label: "THEME",
                    subtitle: themeManager.activeTheme.label
                )
            }
            .buttonStyle(.plain)
