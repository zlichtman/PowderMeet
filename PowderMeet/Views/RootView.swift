...
struct RootView: View {
    ...
    /// Theme accent picker. Held here as @State so SwiftUI tracks
    /// it for any descendants that observe via @Environment. The
    /// previous `.id(themeManager.generation)` rebuild was removed
    /// — it was tearing down sheet state on every theme change,
    /// dismissing the picker before the user could try a second
    /// option. Theme changes now propagate via Swift's Observation
    /// framework: HUDTheme.accent is a computed property that reads
    /// `ThemeManager.shared.activeTheme.accentColor`, and views that
    /// read it in their body register a dep on activeTheme transparently.
    ...
    var body: some View {
        ...
        // Pin the *window* dark only while signed out. The Sign in with
        // Apple sheet is drawn out-of-process and reads the presenting
        // window's interface style (a SwiftUI `.preferredColorScheme`
        // does not reach it), so this is what makes it show the dark
        // app icon. Released to `.unspecified` once authenticated so the
        // scene reports the true Control-Center setting to the theme
        // gallery. Replaces the old app-wide Info.plist override.
        .background(
            WindowOverrideStyle(style: supabase.isAuthenticated ? .unspecified : .dark)
        )
        .sheet(isPresented: Binding(
            get: { supabase.pendingPasswordRecovery },
            set: { supabase.pendingPasswordRecovery = $0 }
        )) {
            // Password-reset deep link landed → collect new password
            // over whatever surface the user happens to be on (auth
            // screen, content, splash). The sheet itself owns the
            // commit and clears the flag on success or explicit
            // dismiss; `interactiveDismissDisabled` keeps the user
            // from accidentally swiping it away mid-flow.
            NewPasswordSheet()
