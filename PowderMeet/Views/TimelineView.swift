...
struct TimelineView: View {
    ...
    // Cached formatters — `body` reads these during a scrub drag (many times
    // per second). Allocating a fresh DateFormatter per call (each parses its
    // format string + resolves locale/calendar) was two allocations per frame.
    ...
    private func fullTime(_ date: Date) -> String {
        Self.fullTimeFormatter.string(from: date).uppercased()
    }
