...
//
//  Fallback (no bundled topsheet): a complete PowderMeet house ski,
//  alpha-trimmed and aspect-fit like the catalog product photos.
//
...
struct SkiPairView: View {
    ...
    /// PowderMeet house ski — used in the picker preview where "no ski
...
private struct HorizontalSkiView: View {
    ...
    private var resolvedAssetKey: String? {
        assetKey ?? (showFallback ? "powdermeet-default" : nil)
    }
    ...
    var body: some View {
        ...
        .onAppear {
            if let key = resolvedAssetKey, trimmed == nil {
                trimmed = TopsheetCache.peek(key)
            }
        }
        .task(id: resolvedAssetKey) {
            guard let key = resolvedAssetKey else {
                trimmed = nil
                return
            }
            var loaded = await TopsheetCache.loadAsync(key)
            if Task.isCancelled { return }
            if loaded == nil, showFallback, key != "powdermeet-default" {
                loaded = await TopsheetCache.loadAsync("powdermeet-default")
            }
            if Task.isCancelled { return }
    ...
    /// ratio is not loaded yet, reserve a 9:1 frame. The house ski
    /// uses its own alpha-trimmed proportions once loaded.
