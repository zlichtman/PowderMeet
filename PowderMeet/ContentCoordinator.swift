...
final class ContentCoordinator {
    ...
    func handleSelectedEntryChange(to entry: ResortEntry?) {
        ...
        // Cancel any in-flight reroute task before nilling the session —
        // a parked `Task.sleep` inside the new GPS-wait loop would wake
        // up after the resort swap and either no-op (best case) or
        // mutate `coord.transientMessage` against the new resort's UI.
