...
nonisolated enum ActiveMeetResortTransitionPolicy {
    static func preservesActiveMeetup(
        activeResortID: String?,
        selectedResortID: String?
    ) -> Bool {
        guard let activeResortID, let selectedResortID else { return false }
        return activeResortID == selectedResortID
    }
}
...
final class ContentCoordinator {
    ...
    var selectedTime: Date = ContentCoordinator.defaultViewTime()

    /// The map opens on a flattering mid-morning light instead of the
    /// real clock — most evenings "now" is dark and a black mountain is
    /// a terrible first impression. Scrubbing the timeline still surfaces
    /// the true day/night + weather cycle; this only seeds the baseline.
    static func defaultViewTime() -> Date {
        let now = Date()
    ...
    @ObservationIgnored var transientMessageClearTask: Task<Void, Never>?
    let transientMessageDurationSeconds: Double = 4.0
    ...
    /// Set during bootstrap when the user's GPS lands inside more than one
    /// catalog bounding box (Vail/Beaver Creek, Park City/Deer Valley,
    /// Mammoth/June). Sorted nearest-first by Euclidean centroid distance.
    /// `ResortPickerSheet` reads this and renders an "AT YOUR LOCATION"
    /// section above the regional groupings so the user picks the right
    /// resort with one tap. Empty when GPS picks a single candidate.
    var pendingResortChoices: [ResortEntry] = []
    ...
    @ObservationIgnored var resortManagerRef: ResortDataManager?
    ...
    /// Keeps canonical closures/lift waits fresh while the app remains open
    /// all day. The pure policy throttles retries; this handle prevents two
    /// overlapping refreshes when a timer tick and scene resume coincide.
    @ObservationIgnored var operationalRefreshTask: Task<Void, Never>?
    @ObservationIgnored var lastOperationalRefreshAttemptAt: Date?
    ...
    @ObservationIgnored var knownFriendIds: Set<UUID> = []
    ...
    func handleSelectedEntryChange(to entry: ResortEntry?) {
        let preservesActiveMeetup = ActiveMeetResortTransitionPolicy
            .preservesActiveMeetup(
                activeResortID: activeMeetSession?.datasetIdentity.resortID,
                selectedResortID: entry?.id
            )
        resortConditions = nil
        if !preservesActiveMeetup {
            meetingResult = nil
        }
        ...
        // Reset the scrubber on every resort change so a fresh mountain
        // doesn't inherit a stale scrubbed instant. Baseline is the
        // flattering daylight default (not the real clock) so switching
        // resorts at night doesn't drop you onto a black mountain;
        // scrubbing still reveals the true day/night + weather cycle.
        // (`isScrubbingTimeline` lives on ContentView's @State; the
        // .onChange handler there has its own reset path.)
        selectedTime = ContentCoordinator.defaultViewTime()
        // A route is resort-specific, but selecting the exact resort owned by
        // an accepted/recovered session is setup, not abandonment. This also
        // closes the race where SwiftUI's delayed onChange fired after strict
        // activation and immediately cancelled the newly-restored request.
        if !preservesActiveMeetup {
            if let sessionId = activeMeetSession?.id {
                meetRequestService.endRequestEventually(sessionId)
            }
            meetRequestService.trackActiveRequest(nil)
            // Cancel any in-flight reroute task before nilling the session — a
            // parked retry must not wake against another resort's UI.
            meetup.cancelActiveSession()
        ...
        lastOperationalRefreshAttemptAt = nil
        ...
        // Pre-warm Mapbox satellite tiles for this resort's bbox so
        // panning around the mountain on a chairlift doesn't stream
        // tiles ("sky loads when you move around"). Fire-and-forget;
        // idempotent within a session. Failures degrade gracefully —
        // tiles still stream the old way if the prewarm errors.
        MapboxOfflineCache.shared.prewarm(resort: entry)
    ...
    func handleLocationChange() {
        ...
        if isRoutingGrade, !userManuallyPickedResort {
        ...
        if isRoutingGrade,
           let myId = SupabaseManager.shared.currentSession?.user.id {
        ...
        // A fresh low-accuracy fix may still broadcast as an honest halo, but
        // it cannot advance, complete, or deviate active navigation.
        guard isRoutingGrade,
              let capturedAt = locationManager.currentFixTimestamp else { return }

        let event = tracker.update(location: coord, at: capturedAt)
        // Recompute on every fix, not only at a graph-node transition. The
        // tracker now measures progress inside the current trail polyline, so
        // maneuver distance and the progress bar should move continuously.
        navigationViewModel?.recompute()

        if let event {
            navigationDirector?.handle(
                event, currentLocation: coord,
                allowRouteFraming: session.guidanceStatus == nil
            )
            switch event {
        ...
        // Off-route movement is not evidence about pace on the accepted path.
        // Keep its estimator untouched while the deviation reroute resolves.
        guard !tracker.isOffRoute, let estimator = etaEstimator else { return }
        let remaining = tracker.remainingRouteMeters
        let sampleTime = Date()
        guard let routeEstimate = meetup.liveRouteETAEstimate(
            session: session,
            at: sampleTime
        ) else {
            // Time can make an unchanged path impossible (for example, a
            // lift's last-loading deadline). Never keep publishing a stale
            // finite ETA while that route is no longer traversable.
            if meetup.rerouteTask == nil {
                transientMessage = "ROUTE CONDITIONS CHANGED — RECALCULATING"
                Task { await meetup.reroute(invalidateIfUnavailable: true) }
            }
            return
        }
        // A fresh on-route fix plus current exact-path validation can restore
        // guidance after a failed GPS recovery. While a replacement is still
        // in flight, do not publish the old plan's ETA as current guidance.
        if var current = activeMeetSession,
           current.id == session.id,
           current.routeRecoveryState == .unavailable,
           meetup.rerouteTask == nil {
            current.routeRecoveryState = .ready
            activeMeetSession = current
        }
        guard activeMeetSession?.routeRecoveryState == .ready else { return }
        ...
        meetup.updateLocalETA(eta)
        meetup.broadcastLocalETAIfNeeded(
    ...
    /// Refreshes mutable canonical status without rebuilding topology. Called
    /// by the foreground clock and after a meaningful background interval.
    /// `force` is reserved for resume, where the app may have slept through
    /// the sidecar's entire validity window.
    func refreshOperationalStatusIfNeeded(now: Date = .now, force: Bool = false) {
        guard operationalRefreshTask == nil,
              let entry = selectedEntry,
              resortManager.currentDataset?.source == .canonicalServer else { return }
        guard force || OperationalRefreshPolicy.shouldRefresh(
            status: resortManager.currentStatus,
            now: now,
            lastAttemptAt: lastOperationalRefreshAttemptAt
        ) else { return }

        lastOperationalRefreshAttemptAt = now
        operationalRefreshTask = Task { @MainActor [weak self] in
            guard let self else { return }
            await self.resortManager.loadResort(
                entry,
            ...
            self.meetup.revalidateActiveSessionAfterOperationalRefresh(now: .now)
            self.operationalRefreshTask = nil
        }
    }
    ...
    func handleScenePhaseChange(oldPhase: ScenePhase, newPhase: ScenePhase) {
        ...
        if let entry = selectedEntry {
            if resortManager.currentDataset?.source == .canonicalServer {
                // Topology is immutable, operational state is not. Force a
                // sidecar refresh after a real background interval so an old
                // in-memory closure or lift wait cannot survive indefinitely.
                refreshOperationalStatusIfNeeded(force: true)
            }
            Task {
