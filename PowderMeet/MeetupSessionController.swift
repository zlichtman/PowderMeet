...
final class MeetupSessionController {
    ...
    func reroute(fasterOnlyByThresholdSeconds: Double? = nil) async {
        rerouteTask?.cancel()
        let task: Task<Void, Never> = Task { [weak self] in
            await self?.performReroute(fasterOnlyByThresholdSeconds: fasterOnlyByThresholdSeconds)
        }
        rerouteTask = task
        await task.value
        if rerouteTask == task { rerouteTask = nil }
    }

    private func performReroute(
        fasterOnlyByThresholdSeconds: Double?
    ) async {
        guard let coord = coordinator,
        ...
        let isDeviationReroute = fasterOnlyByThresholdSeconds == nil
        ...
        let solver = configureSolver(graph: graph)
        var myNodeId: String?
        var myRoute: (path: [GraphEdge], time: Double)?
        var attemptIdx = 0
        ...
        while myRoute == nil {
            ...
            if myNodeId == nil,
               let myCoord = coord.locationManager.currentLocation,
               let myNode = graph.nearestNode(to: myCoord) {
                myNodeId = myNode.id
            }

            if let id = myNodeId {
                myRoute = solver.pathTo(target: target, from: id, skier: myProfile)
                if myRoute != nil { break }
                AppLog.meet.info("reroute: no path from \(id) — backoff \(attemptIdx)")
            } else {
        ...
        guard let myNodeId, let myRoute else { return }

        // Threshold gate (live-faster-route case): only swap when the
        // newly-computed path is meaningfully faster than what's
        // remaining on the current path. Used by the graph-fingerprint
        // watcher so opening up a previously-closed run/lift doesn't
        // produce a flickering route line if the time delta is noise.
        if let threshold = fasterOnlyByThresholdSeconds {
            let context = solver.makeContext(for: myProfile.id.uuidString)
            let remainingEta = remainingEtaSeconds(
                session: session, profile: myProfile, context: context
            )
            let delta = remainingEta - myRoute.time
            if delta < threshold {
                AppLog.meet.debug("reroute:rejected:reason=below-threshold delta=\(Int(delta))s threshold=\(Int(threshold))s")
                return
            }
            AppLog.meet.debug("reroute:applied:delta=\(Int(delta))s threshold=\(Int(threshold))s")
        ...
        if isDeviationReroute,
           coord.transientMessage == "RECALCULATING ROUTE…" || coord.transientMessage == "ROUTE STALE — WAITING FOR GPS" {
            coord.transientMessage = nil
        }
    ...
    /// lines.
    @ObservationIgnored private var lastFasterRerouteAt: Date = .distantPast
    private let fasterRerouteThrottle: TimeInterval = 30.0
    ...
    /// kicks `reroute(fasterOnlyByThresholdSeconds:)` with an adaptive
    /// threshold so a meaningfully faster newly-opened path swaps in
    /// but numerical noise doesn't.
    func evaluateFasterRerouteIfNeeded() {
        ...
        // Skip when the user is on the final leg — re-routing now
        // would just swap one final-edge for another and confuse them
        // when arrival is seconds away. 30 s matches the lower bound
        // of the swap threshold below.
        if remaining <= 30 {
            return
        }
        lastFasterRerouteAt = now
        // Adaptive threshold: a 5-minute remaining route should need
        // ≥30 s improvement (10 % of 300 s = 30 s); a 20-minute one
        // needs ≥120 s (10 % dominates). Floor at 30 s so very short
        // remaining routes don't trigger on tiny deltas.
        let threshold = max(30, remaining * 0.10)
        // Keep the tracker reference live for the closure
        _ = tracker
        Task { [weak self] in
            await self?.reroute(fasterOnlyByThresholdSeconds: threshold)
        }
    }
    ...
    private func remainingEtaSeconds(
        ...
        for edge in tracker.path[tracker.currentEdgeIndex...] {
            if let t = profile.traverseTime(for: edge, context: context) {
                total += t
            }
        }
        return total
