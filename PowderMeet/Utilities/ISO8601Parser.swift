...
nonisolated enum ISO8601Parser {
    ...
    /// Format a `Date` as an ISO 8601 string with fractional seconds — the
    /// flavor Postgres `timestamptz` round-trips cleanly. Callers that used
    /// to spin up a one-off `ISO8601DateFormatter` for `string(from:)` route
    /// through here.
