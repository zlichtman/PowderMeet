...
struct ImportedRunRecord: Codable, Identifiable {
    ...
    /// SHA256 of the source file's bytes. Preserved as raw provenance and
    /// round-tripped so restored rows retain their original source identity.
    let sourceFileHash: String?
    /// Immutable dataset identity and full stable segment attribution used
    /// by the matcher. Optional for rows created before the provenance schema.
    let datasetVersion: String?
    let matchedSegmentIds: [String]?
    /// Edge-local timing evidence used by personalized routing. Optional for
    /// rows written before export schema v5.
    ...
    /// Uppercased label for the source badge ("SLOPES", "GPX", "HEALTH", …).
    /// Empty string when source is nil so the badge can be hidden.
    ///
    /// Resolves through `ImportSource.displayName` rather than naively
    /// uppercasing the persisted string: the DB stores the raw value
    /// (`"healthkit"`), whose uppercase ("HEALTHKIT") is NOT the badge
    /// label ("HEALTH") and doesn't match the viewer's color map — that
    /// mismatch is exactly why the Apple-Health tag rendered grey and
    /// over-long. Going through `displayName` keeps badge text + color
    /// correct for every source whose raw value differs from its label.
    var sourceBadge: String {
        guard let source else { return "" }
        return ImportSource(rawValue: source)?.displayName ?? source.uppercased()
    }
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case matchedSegmentIds = "matched_segment_ids"
        case edgePaceObservations = "edge_observations"
        case matchConfidence = "match_confidence"
        case matchMethod = "match_method"
        case equipmentIdAtActivity = "equipment_id_at_activity"
        case rawSourceIdentity = "raw_source_identity"
    }
}
