...
nonisolated extension MountainGraph {
    ...
    func runTrailGroupId(containingRunNode nodeId: String) -> String? {
        runs
            ...
            .map { $0.attributes.trailGroupId ?? $0.id }
            .min()
    }
...
nonisolated extension RunTrailGroupSummary {
    fileprivate static func build(
        ...
        // A group can include unnamed junction fragments. Geometry ordering
        // must not erase a real name carried by another member of the group.
        // Use that same stable order to preserve deterministic disambiguation.
        let displayName = ordered.lazy.compactMap { edge -> String? in
            guard let raw = edge.attributes.trailName else { return nil }
