...
struct ResortConditions: Sendable {
    /// Weather is advisory rather than a routing-availability gate, but old
    /// wind/visibility/snow values must not silently alter a live ETA.
    nonisolated static let routingFreshnessWindow: TimeInterval = 45 * 60
    nonisolated static let allowableFutureClockSkew: TimeInterval = 5 * 60
    ...
    /// Resort-local offset returned by Open-Meteo for the fetched instant,
    /// including daylight saving time. nil only for legacy/test snapshots.
    ...
    nonisolated func isFreshForRouting(at date: Date = .now) -> Bool {
        let age = date.timeIntervalSince(fetchedAt)
        return age >= -Self.allowableFutureClockSkew
            && age <= Self.routingFreshnessWindow
    }
