...
nonisolated struct UserProfile: Codable, Identifiable, Sendable, Equatable {
    ...
    /// "no path" message.
    ///
    /// `arrivalTimeOffsetSeconds` lets Dijkstra pass the cumulative
    /// path time so far so lift wait calculations are evaluated at
    /// the time the skier *arrives* at the lift, not at the time the
    /// solver runs. Without this, a 14-minute path to a busy lift
    /// uses the now-wait instead of the now+14-minutes wait, and the
    /// recommendation lands the user in a queue that's grown since.
    ...
    func traverseTime(
        ...
        case .lift:
            ...
            let arrivalTime: Date? = context.solveTime.map {
