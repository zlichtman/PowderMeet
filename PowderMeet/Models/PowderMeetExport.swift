...
//    v3: profile + stats + runs + avatar bytes
//    v4: run dataset/match/equipment provenance
//    v5: edge-attributed pace observations (current)
//
...
struct ImportedRunBackup: Codable {
    ...
    init(from record: ImportedRunRecord) {
        ...
        matchedSegmentIds = record.matchedSegmentIds
        edgePaceObservations = record.edgePaceObservations
        matchConfidence = record.matchConfidence
        matchMethod = record.matchMethod
        equipmentIdAtActivity = record.equipmentIdAtActivity
        rawSourceIdentity = record.rawSourceIdentity
    }
}
