...
nonisolated struct MountainNaming {
    ...
    func nodeLabel(_ nodeId: String, style: Style = .canonical) -> String {
        ...
        if let groupId = cache.runGroupForNode[nodeId],
           let title = cache.displayTitleByGroupId[groupId] {
            // `.withChainPosition` previously suffixed " · TOP" /
            // " · BOTTOM" on chain-endpoint nodes. Per user direction
            // the position suffix isn't needed — the map context
            // already shows where on the chain you are. Collapsed
            // into the same display title as `.canonical`.
            switch style {
    ...
    func edgeLabel(_ edge: GraphEdge, style: Style = .canonical) -> String {
        ...
        if let raw = edge.attributes.trailName, !raw.isEmpty {
            switch style {
            case .canonical, .withChainPosition:
                // The difficulty chip + the colored route line already
                // encode the level visually, so the textual " · Black"
                // suffix is redundant. Keep the trail name only,
                // and strip a leading color word from it if it
                // matches the trail's actual difficulty (e.g. trail
                // "Black Magic" with difficulty=black → "Magic"). If
                // the leading color word doesn't match the difficulty
                // it's just part of the trail's real name (e.g. "Black
                // Forest" rated green) and must be preserved.
                if let diff = edge.attributes.difficulty, edge.kind == .run {
                    return Self.stripRedundantDifficultyPrefix(from: raw, matching: diff)
                }
                return raw
            case .bareName:
                return raw
        ...
        // Nothing inherited — fall back to a generic "Run" label for
        // runs (the chip + route color encode the level redundantly,
        // so the prior "Black Run" / "Blue Run" textual prefix is
        // dropped) instead of the useless "Junction (1234m)" the
        // kind+elevation fallback would produce. For lifts, fall
        // through to the standard nodeLabel path so existing lift-
        // naming heuristics still apply.
    ...
    /// If the leading word of `name` matches the trail's difficulty
    /// (e.g. trail "Black Magic" with difficulty=black), drop the
    /// redundant prefix. Otherwise return the name unchanged so
    /// trails whose names happen to contain a non-matching color
    /// word ("Black Forest" rated green) keep their full identifier.
    /// Case-insensitive prefix match; preserves the original case of
    /// the surviving text.
    private static func stripRedundantDifficultyPrefix(
        from name: String,
        matching difficulty: RunDifficulty
    ) -> String {
        let candidates: [(prefix: String, diff: RunDifficulty)] = [
            ("Green ",         .green),
            ("Blue ",          .blue),
            ("Black ",         .black),
        ...
        where candidate.diff == difficulty
            && lower.hasPrefix(candidate.prefix.lowercased())
        {
            let stripped = name.dropFirst(candidate.prefix.count)
                .trimmingCharacters(in: .whitespaces)
            // Refuse to return an empty string — if the entire trail
            // name is just the color word, fall through to the full
            // original.
    ...
    func meetingNodeLabel(_ nodeId: String) -> String {
        ...
        // A rendezvous is a landmark, not merely a point that happens to lie
        // on a run chain. Lift bases commonly share the bottom node of a named
        // trail; general `nodeLabel` correctly prefers the run for navigation,
        // but "meet at Excelerator Express Base" is far more recognizable than
        // "meet at Excelerator Chairlift Access · Blue".
        if let lift = cache.liftLabelByNodeId[nodeId] {
            switch node.kind {
            case .liftBase:
                return "\(lift) · Base"
            case .midStation:
                return "\(lift) · Mid-Station"
            default:
                break
            }
        }
        return nodeLabel(nodeId, style: .canonical)
    }

    /// Receiver-friendly variant of `meetingNodeLabel`. Returns `nil`
    /// when the node isn't in this graph — caller should fall through
    /// to a generic "Meeting Point" string instead of leaking the raw
    /// node id (which looks like `n45123_122456` and reads as garbage
    /// in the UI). Topology divergence between sender and receiver
    /// graphs (different snapshot dates, different curated overlays)
    /// is the most common cause of a node id miss; the snapshot-
    /// mismatch confirm sheet covers the user-facing warning.
    func meetingNodeLabelOrNil(_ nodeId: String) -> String? {
        guard graph.nodes[nodeId] != nil else { return nil }
        return meetingNodeLabel(nodeId)
    }
...
fileprivate nonisolated final class NamingCache {
    ...
    static func build(for graph: MountainGraph) -> NamingCache {
        ...
        // unnamed lifts get a stable "<Type> #N" by canonical edge ID.
        var liftLabelByNodeId: [String: String] = [:]
        var liftPickerEntries: [(nodeId: String, label: String)] = []
        let stableLabelKey: (String) -> String = { value in
            value.folding(
        ...
        for edge in graph.edges where edge.kind == .lift {
            ...
            if let existing = namedLifts[key] {
                if existing.elevation < srcElev { continue }
                if existing.elevation == srcElev,
                   existing.sourceId <= edge.sourceID { continue }
            }
            namedLifts[key] = (edge.sourceID, edge.targetID, srcElev, name)
        }
        let orderedNamedLifts = namedLifts.values.sorted {
            let lhsName = stableLabelKey($0.displayName)
            let rhsName = stableLabelKey($1.displayName)
            if lhsName != rhsName { return lhsName < rhsName }
            if $0.elevation != $1.elevation { return $0.elevation < $1.elevation }
            return $0.sourceId < $1.sourceId
        }
        for info in orderedNamedLifts {
        ...
        // Unnamed: stable index by canonical edge ID.
        //
        // Only emit a picker entry when the lift edge's source is a
        // `.liftBase` AND that base hasn't already been picker-listed
        // (under a named or earlier unnamed lift). Without these two
        // filters, a single physical lift drawn as multiple OSM ways
        // (split at intermediate towers — common for big gondolas)
        // produces N picker rows for what's actually one chairlift.
        // The labels increment ("Quad #1", "Quad #2", "Quad #3") so
        // they look like distinct lifts at a glance even though
        // they're the same physical machine.
        var listedLiftBases: Set<String> = Set(liftPickerEntries.map { $0.nodeId })
        var unnamedCount = 0
        let orderedLiftEdges = graph.edges
            .filter { $0.kind == .lift }
            .sorted { $0.id < $1.id }
        for edge in orderedLiftEdges {
            ...
            // Stamp the side-label map on every node so map labels
            // resolve correctly mid-lift (this is unrelated to picker
            // emission; the picker filter is below).
            ...
            // Picker emission: lift-base only (so multi-segment chains
            // collapse to one row), and only the first time we see a
            // given base node (so no id collisions when iteration order
            // touches the same base twice).
            guard graph.nodes[edge.sourceID]?.kind == .liftBase else { continue }
            guard !listedLiftBases.contains(edge.sourceID) else { continue }
            listedLiftBases.insert(edge.sourceID)
        ...
        for edge in graph.edges.filter({ $0.kind == .run }).sorted(by: { $0.id < $1.id }) {
        ...
        return NamingCache(
            ...
            liftPickerEntries: liftPickerEntries.sorted {
                let lhsLabel = stableLabelKey($0.label)
                let rhsLabel = stableLabelKey($1.label)
